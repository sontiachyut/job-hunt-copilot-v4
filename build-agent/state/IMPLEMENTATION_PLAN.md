...
- Explicit blocker note: the remaining BA-10 hardening gaps are now explicit and narrower: maintenance automation, chat review/control behavior, and idle-timeout resume.
...
- `BA-10-S3` is in progress: explicit hardening regressions now verify unsupported tailoring asks stay as Step 4 gaps, role-targeted drafting ignores raw-source claims outside the approved tailoring inputs, drafting requires an approved tailoring run, outreach-message traceability exposes reply summaries without raw reply excerpts, runtime secret values stay out of canonical state/handoff artifacts/review outputs, delayed feedback now has a dedicated `launchd` scheduler path through `job_hunt_copilot.local_runtime`, `scripts/ops/run_feedback_sync.py`, `bin/jhc-feedback-sync-cycle`, and `ops/launchd/job-hunt-copilot-feedback-sync.plist`, supervisor heartbeats now use `pmset -g log` first with a >1 hour cycle-gap fallback to detect sleep/wake recovery state, targeted supervisor regressions now confirm downstream retries keep the same durable run id, blocked stage, and pending review packet while the narrow action catalog still blocks later-stage execution, `jhc-chat` now materializes a persisted clean-first startup dashboard plus grouped review snapshot with runtime totals derived only from active supervisor-cycle execution time, and the new posting-abandon control path now marks canonical postings as `abandoned`, records transition plus override history, and retires non-terminal pipeline runs cleanly; the BA-10 trace matrix, blocker audit, and validation suite now reflect the narrower blocker surface at 197 implemented, 3 partial, and 12 gap scenarios while leaving richer in-chat review/control, idle-timeout resume, and maintenance automation explicit.
- Explicit implementation note: `job_hunt_copilot.blocker_audit` and `job_hunt_copilot.quality_validation` now resolve the active BA-10 focus directly from the remaining open gap clusters, so `tests/test_blocker_audit.py` and `tests/test_quality_validation.py` fail if the persisted BA-10-S3 focus drifts away from those remaining maintenance, chat, and idle-timeout-resume gaps.
- Explicit implementation note: the current chat surface is now validated through wrapper lifecycle, explicit-close resume, explicit manual resume after unexpected exit, generated bootstrap guidance to persisted review/control surfaces, and a persisted clean-first startup dashboard plus grouped review snapshot, but in-chat review retrieval, control routing, default change summaries, and idle-timeout auto-resume remain downstream work.
...
- Explicit implementation note: reviewer-facing repo surfaces now name the three remaining BA-10 hardening gaps directly and point at the committed acceptance trace, blocker audit, and latest validation-suite snapshot, keeping the repo overview honest without claiming those runtime behaviors exist.
...
- Explicit implementation note: the acceptance matrix now reports 197 implemented / 3 partial / 12 gap scenarios; the open downstream-supervisor gap cluster is gone, `jhc-chat` startup dashboard plus active-runtime-metrics are now implemented, posting-abandon control is now implemented, and the remaining BA-10 partials belong to maintenance automation, richer chat review/control, and idle-timeout resume.
...
- Why next: BA-10-S4 closed the downstream supervisor action-catalog gap, the latest BA-10-S3 hardening slice burned down the `jhc-chat` startup dashboard plus active-runtime-metrics scenarios, and this cycle closed posting-abandon control, so the highest-value remaining work is the BA-10-S3 burn-down across maintenance automation, richer chat review/control, and idle-timeout resume with refreshed BA-10 evidence.
- Done when:
  - targeted regressions either close or keep explicit the remaining maintenance, chat, and idle-timeout-resume gaps with fresh evidence
