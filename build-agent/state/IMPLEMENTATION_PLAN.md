...
- Why next: the delayed-feedback scheduler and sleep/wake recovery slices are closed and the downstream-supervisor blocker now has fresh retry/idempotency evidence while the acceptance matrix still sits at 190 implemented / 8 partial / 14 gap scenarios, so the next highest-value work is either build-lead implementation of downstream action-catalog steps beyond `lead_handoff` or the next quality-owned blocker confirmation pass across chat review/control.
- Done when:
