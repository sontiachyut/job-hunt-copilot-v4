...
- Why next: the delayed-feedback scheduler, sleep/wake recovery, downstream-supervisor blocker evidence, chat lifecycle/control regressions, and maintenance/posting-abandon blocker evidence are now current while the acceptance matrix still sits at 190 implemented / 8 partial / 14 gap scenarios, so the next highest-value work is build-lead implementation of downstream action-catalog steps beyond `lead_handoff` or a real posting-abandon control surface, with maintenance automation left as the next explicit follow-up gap.
- Done when:
  - remaining BA-10 gaps around downstream supervisor orchestration, chat review/control, maintenance automation, and posting-abandon control are either fixed or reaffirmed as explicit blockers with current evidence
