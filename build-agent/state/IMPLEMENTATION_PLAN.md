...
- Explicit blocker note: the remaining BA-10 hardening gaps are now explicit and narrower: the contact-rooted general-learning supervisor selector path, chat review/control behavior, maintenance automation, and posting-abandon control.
...
- `BA-10-S4` is still in progress as a build-lead implementation slice: `job_hunt_copilot.supervisor` now advances durable role-targeted runs from `lead_handoff` into `agent_review`, reuses the existing tailoring review persistence to approve pending tailored runs into `people_search` or `sending`, registers bounded `people_search`, `email_discovery`, `sending`, and `delivery_feedback` actions for the downstream role-targeted flow, and now also registers bounded contact-rooted `general_learning_email_discovery` plus `general_learning_outreach` actions so no-email general-learning contacts can become send-ready and advance into the existing contact-rooted send path without creating a pipeline run.
...
- Explicit implementation note: the downstream-supervisor gap in `job_hunt_copilot.acceptance_traceability` and `job_hunt_copilot.blocker_audit` now carries a machine-readable implementation snapshot listing the current selector priority order (`active_incident`, `open_pipeline_run`, `new_role_targeted_posting`, `general_learning_contact`), the registered role-targeted checkpoint stages (`agent_review`, `lead_handoff`), the registered role-targeted action stages (`agent_review`, `delivery_feedback`, `email_discovery`, `lead_handoff`, `people_search`, `sending`), the registered contact-rooted action ids (`general_learning_email_discovery`, `general_learning_outreach`), no remaining blocked role-targeted stages, and the still-missing delayed-feedback follow-through path for sent contact-rooted outreach.
- Explicit implementation note: `tests/test_supervisor_downstream_actions.py` now proves that a no-email contact-rooted general-learning candidate is selected for bounded email discovery, becomes send-ready in canonical contact state, is then selected for the existing contact-rooted send path on the next heartbeat without a pipeline run or role-targeted review gate, and that a new eligible role-targeted posting still outranks that opportunistic contact work in the selector order.
- Explicit implementation note: the acceptance matrix now reports 194 implemented / 4 partial / 14 gap scenarios, because this cycle closed the general-learning review-bypass partial while leaving only the deeper contact-rooted follow-through gap explicit.
- Explicit implementation note: the downstream-supervisor gap is now narrowed from contact-rooted email discovery plus delayed-feedback follow-through to delayed-feedback follow-through alone; the next best work is another build-lead BA-10-S4 pass that adds bounded feedback follow-through for sent contact-rooted general-learning outreach without regressing the role-targeted chain.
...
- Why next: the role-targeted supervisor chain now runs end to end through bounded `delivery_feedback`, the supervisor can now also advance contact-rooted general-learning work from bounded email discovery into send-ready dispatch, the focused downstream regressions and BA-10 reports are refreshed, and the acceptance matrix still sits at 194 implemented / 4 partial / 14 gap scenarios; the highest-value remaining work in this same slice is delayed-feedback follow-through for sent contact-rooted outreach.
- Done when:
  - the remaining delayed-feedback follow-through path for sent contact-rooted outreach is implemented or explicitly re-bounded with fresh evidence
  - targeted supervisor and smoke regressions prove the new stage advancement, or the remaining contact-rooted follow-through blocker stays explicit with refreshed evidence
