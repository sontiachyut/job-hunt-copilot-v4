...
- Why next: the delayed-feedback scheduler, sleep/wake recovery, downstream-supervisor blocker evidence, and chat lifecycle/control regressions are all now tightened while the acceptance matrix still sits at 190 implemented / 8 partial / 14 gap scenarios, so the next highest-value work is either build-lead implementation of downstream action-catalog steps beyond `lead_handoff` or the next quality-owned blocker confirmation pass across maintenance automation and posting-abandon control while chat review/control remains an explicit product gap.
- Done when:
