...
- Explicit blocker note: the remaining BA-10 hardening gaps are now explicit and narrower: maintenance automation and chat review/control behavior.
...
- Explicit implementation note: the current chat surface is now validated through wrapper lifecycle, explicit-close resume, explicit manual resume after unexpected exit, safe 15-minute idle-timeout auto-resume, generated bootstrap guidance to persisted review/control surfaces, and a persisted clean-first startup dashboard plus grouped review snapshot, but in-chat review retrieval, control routing, and default change summaries remain downstream work.
- Explicit implementation note: `tests/test_local_runtime.py` now also seeds multiple pending review packets, failed expert-requested background tasks, maintenance batches, and open incidents to prove the persisted `jhc-chat` review snapshot stays grouped in the documented order, newest-first within each group, and compact-first via `max_items_per_group`, while explicit chat-time retrieval and control routing still remain backlog.
...
- Explicit implementation note: the acceptance matrix now reports 198 implemented / 2 partial / 12 gap scenarios; the open downstream-supervisor gap cluster is gone, `jhc-chat` startup dashboard plus active-runtime-metrics are now implemented, posting-abandon control is now implemented, idle-timeout resume is now implemented, and the remaining BA-10 partials belong to maintenance automation and richer chat review/control.
...
- Why next: BA-10-S4 closed the downstream supervisor action-catalog gap, the latest BA-10-S3 hardening slices burned down the `jhc-chat` startup dashboard plus active-runtime-metrics scenarios, posting-abandon control, and idle-timeout auto-resume, so the highest-value remaining work is the BA-10-S3 burn-down across richer chat review/control and maintenance automation with refreshed BA-10 evidence.
- Done when:
