...
- the current Gmail-intake slices under `job_hunt_copilot.gmail_alerts` and `job_hunt_copilot.linkedin_scraping`, including `jhc-linkedin-ingest gmail-batch`, persisted collected-email units under `linkedin-scraping/runtime/gmail/{received_at}-{gmail_message_id}/`, plain-text-first multi-card parsing with HTML fallback only when needed, zero-card review-threshold metadata retained in `email.json`, and autonomous per-card lead fan-out into canonical lead workspaces with `alert-email.md`, `alert-card.json`, `jd-fetch.json`, `lead-manifest.yaml`, conservative dedupe, and honest `incomplete` / `blocked_no_jd` lead state

What is still in progress:
- Gmail JD-source merge and mismatch review, plus the later runtime components on top of the now-landed local operator, manual-ingestion/materialization/refresh-history path, and autonomous Gmail collection plus lead fan-out entrypoints
