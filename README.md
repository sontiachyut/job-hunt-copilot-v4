...
- the current Resume Tailoring bootstrap slices under `job_hunt_copilot.resume_tailoring`, including `job_posting_id`-rooted eligibility evaluation from persisted `jd.md`, application-local `applications/{company}/{role}/eligibility.yaml` audit artifacts, honest `hard_ineligible` posting short-circuiting, initial `resume_tailoring_runs` bootstrap or reuse, per-posting workspace scaffolding under `resume-tailoring/output/tailored/{company}/{role}/`, shared-contract `meta.yaml`, mirrored `jd.md` / optional `post.md` / `poster-profile.md`, working `resume.tex`, `scope-baseline.resume.tex`, component-local input mirrors, and Step 3 through Step 7 intelligence scaffold artifacts

What is still in progress:
- Resume Tailoring intelligence generation, finalize or compile or one-page verification, mandatory agent review, outreach execution, review surfaces, and the later runtime components on top of the now-landed local operator plus completed manual and Gmail ingestion paths
