...
- the first Resume Tailoring slice under `job_hunt_copilot.resume_tailoring`, including `job_posting_id`-rooted eligibility evaluation from persisted `jd.md`, application-local `applications/{company}/{role}/eligibility.yaml` audit artifacts, honest `hard_ineligible` posting short-circuiting, initial `resume_tailoring_runs` bootstrap or reuse, and recorded state-transition history for eligibility gating plus run creation

What is still in progress:
- Resume Tailoring workspace scaffolding, Step 3 through Step 7 intelligence generation, finalize or compile or one-page verification, mandatory agent review, outreach execution, review surfaces, and the later runtime components on top of the now-landed local operator plus completed manual and Gmail ingestion paths
