...
- repo-local launchd wiring under `ops/launchd/` plus `bin/jhc-agent-start`, `bin/jhc-agent-stop`, `bin/jhc-agent-cycle`, `bin/jhc-feedback-sync-cycle`, and `bin/jhc-chat` for local heartbeat, delayed feedback polling, and operator control
...
- the current BA-10 quality layer under `build-agent/reports/` and `scripts/quality/`, including the committed acceptance trace matrix, blocker audit, smoke harness, and a reproducible `run_ba10_validation_suite.py` entrypoint for the automated hardening checks
