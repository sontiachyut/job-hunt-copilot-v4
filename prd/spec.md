...
- **FR-EM-12C5D (Technical Path Fixed Paragraphs Rule):** Technical Paragraph 2 through Paragraph 4 shall remain deterministic and fixed in wording for the current build, including the Azure / `~580 TPS` / `99.95% uptime` background paragraph, one concise Job Hunt Copilot paragraph with the repo folded into the project mention rather than a standalone `repo is here` sentence, and a concise guidance ask without explicit scheduling availability text.
