...
- Apollo email enrichment for seeded contacts plus Apollo current-company manager-expansion harvest
...
10. Apollo current-company manager-expansion harvest when additional leadership contacts are useful for the posting, plus Apollo enrichment for the kept contact set
...
9. The promoted posting does not maintain a separate minimum total-contact target for Apollo. All Jobright-seeded contacts stay in scope, and Apollo additions are governed by the separate manager-expansion shortlist policy.
10. If the reachable current-company contact set remains small after provider coverage is exhausted, the posting may still proceed with that smaller reachable set rather than remaining blocked on an arbitrary floor.
11. Apollo current-company manager / executive / founder harvest may shortlist additional leadership contacts whenever those shortlisted contacts look useful for role-targeted outreach.
...
27. The system may reopen replacement selection only when the current intended outreach set degrades, becomes unusable, or falls below the needed size because of enrichment failures, repeat-contact exclusions, or other concrete loss of usable contacts.
...
2. For Apollo company-scoped manager-expansion search:
   - company name
...
5. Apollo current-company search should remain bounded and role-relevant. It should not gather a broad unbounded company pool by default; it should harvest shortlistable current-company manager / executive / founder contacts under the adaptive cap.
...
2. Source-seeded Jobright contacts plus Apollo-shortlisted manager-expansion contact candidates, with discovered email candidate(s) and confidence metadata
...
11. Run Apollo manager-expansion search when additional current-company leadership contacts are useful, and enrich the added contacts
...
- **FR-SYS-01G2 (Apollo Addition Rule):** Apollo shall not maintain a separate minimum-total-contact top-up rule. All Jobright-seeded contacts remain first-class, and Apollo shall add only the separately shortlisted current-company contacts selected by the manager-expansion policy.
...
- **FR-SYS-01G2C (Frozen Intended Contact Set Rule):** Once the initial intended outreach set has been built from Jobright-seeded contacts plus any Apollo-shortlisted manager-expansion contacts, that set shall remain frozen by default rather than auto-expanding whenever new candidate contacts later appear.
- **FR-SYS-01G2D (Refill-On-Degradation Rule):** The system may reopen replacement selection only when the current intended outreach set degrades, becomes unusable, or loses usable contacts because of enrichment failures, repeat-contact exclusions, or other concrete loss of usable contacts.
- **FR-SYS-01G2E (Removed-Contact Frontier Exclusion Rule):** Once a `job_posting_contacts` link leaves the intended outreach set, that removed link shall no longer participate in automatic send-set selection, active-wave loading, or person-scoped role-targeted email discovery for that posting. Historical message rows remain preserved for audit history, but removed contacts must not continue consuming the current frontier.
- **FR-SYS-01G2A (Smaller Reachable Set Continuation Rule):** A posting shall be allowed to continue with whatever reachable current-company set exists after provider coverage is exhausted; there is no separate 5-contact floor.
- **FR-SYS-01G3 (Apollo Current-Company-Only Rule):** Apollo-added contacts shall be current-company contacts for the target employer and shall not add external alumni-style or unrelated out-of-company contacts.
...
- **FR-SYS-01G5A (Leadership-Adjacent Technical Title Rule):** For engineering-role Apollo expansion and later role-targeted send ordering, titles such as `Founding Engineer`, `Technical Team Lead`, `Software Team Lead`, `Lead Software Engineer`, `Lead Engineer`, `Lead Machine Learning Engineer`, and similar current-company technical-lead titles shall be treated as leadership-adjacent contacts rather than plain engineers when they clearly imply team or technical-lead responsibility.
- **FR-SYS-01G5B (True-Manager Priority Rule):** Leadership-adjacent technical-lead titles remain below true people-manager and executive buckets. Engineering managers, hiring managers, founder / co-founder executive contacts, director / head / vice-president engineering leadership, and relevant technical CxO contacts shall still outrank leadership-adjacent technical-lead titles in the current manager-first lane.
...
- **FR-SYS-17B1 (Required Linked Contacts Meaning):** For this build, `required linked contacts` should mean:
  ...
  2. preserve all viable source-seeded contacts and shortlist useful Apollo manager-expansion contacts when applicable
...
- **FR-SYS-17C (Ready-for-Outreach Meaning):** A `job_posting` shall be considered `ready_for_outreach` when:
  ...
  3. the posting has the current intended outreach set for that job, including all viable source-seeded contacts plus any Apollo-shortlisted manager-expansion contacts selected for that posting
...
- **FR-SYS-17K (Requires-Contacts-to-Ready Transition):** A `job_posting` shall transition from `requires_contacts` to `ready_for_outreach` when the required linked contacts exist and at least one contact in the current intended outreach set has discovered a usable email address and is otherwise automatically send-eligible.
...
- **FR-SYS-33B (Apollo Promoted-Latest-Field Rule):** The lightweight Apollo anchor fields stored directly on `contacts` shall be a promoted latest-known projection for operational lookup. The full provider payload remains in provider-snapshot storage, while the core contact row keeps only the latest operational fields most useful for identity, drafting, and routing.
...
- **FR-SYS-38 (Primary Orchestration Sequence):** The primary role-targeted workflow shall run in this dependency order: Lead Ingestion -> promotion gate -> eligibility/tailoring -> mandatory agent review of the tailored output -> source-seeded contact enrichment -> Apollo company-scoped search when the posting is intentionally running manager-expansion or later replacement selection -> selected-contact recipient-profile extraction -> email discovery for contacts still missing usable emails -> drafting/sending -> delivery feedback.
...
- **FR-SYS-38A3C (Apollo Full-Payload Persistence Rule):** When Apollo returns candidate or enrichment data for a shortlisted/materialized contact, the system shall persist the full Apollo person payload returned at that stage inside the main database in addition to preserving the search/enrichment artifacts. The database-backed provider snapshot shall retain the full Apollo response shape rather than only a hand-picked subset of fields.
- **FR-SYS-38A3D (Apollo Employment-History Persistence Rule):** When Apollo returns employment history for a materialized contact, that employment history shall be persisted in structured database form and linked to the canonical `contact_id` so downstream drafting and later review can query it without reparsing raw artifacts.
...
- **FR-SYS-38B3H (Orphaned Send-Stage Recovery Priority Rule):** When an orphaned downstream recovery can resume actionable sending now, that recovery shall outrank orphaned feedback-only cleanup and ordinary open `email_discovery` or `people_search` backlog. Feedback-only orphan recovery may run before discovery, but it shall not keep a send-actionable orphan stranded.
- **FR-SYS-38B3I (Single-Shot Feedback-Only Recovery Rule):** Feedback-only orphan recovery exists only to recreate one missing downstream closure run. Once a posting has already completed a downstream delivery-feedback closure run, the supervisor shall not keep re-bootstraping new feedback-only recovery runs for that same posting unless new actionable send-stage work appears later.
- **FR-SYS-38B3J (Fresh-Posting Bootstrap Over Ordinary Discovery Backlog):** When no actionable send-stage work, incident work, orphaned send-stage recovery, or other higher-priority non-discovery run is due, the supervisor shall prefer bootstrapping an eligible `sourced` or `resume_review_pending` posting that has no non-terminal `pipeline_run` over resuming ordinary open role-targeted `email_discovery` or `people_search` backlog. This prevents older unresolved `requires_contacts` postings from monopolizing heartbeats while fresher postings never enter the pipeline.
- **FR-SYS-38B3K (Ordinary Discovery Yield Rule):** A role-targeted `email_discovery` or `people_search` run that is merely continuing ordinary unresolved contact discovery shall remain resumable later, but it shall yield heartbeat selection to eligible fresh-posting bootstrap when the queue otherwise has no send-ready frontier. The system shall not globally pause the pipeline just because the older `requires_contacts` backlog still lacks untouched contacts with usable emails.
...
- **FR-SYS-38H1F5B (Window-Wait Versus Gap-Wait Reporting):** Follow-up worker summaries and related queue diagnostics shall distinguish candidates waiting because of the active shared send window from candidates waiting because of the global inter-send pacing gap. A single combined waiting count is not sufficient for the current 24-hour alternating-window policy.
...
- **FR-SYS-38H1G (Follow-Up Rollout Gate):** The first live rollout of the follow-up worker shall go directly into bounded automatic sending when the owner explicitly enables it. A pre-rollout dry-run validation phase is not required by default for this feature.
- **FR-SYS-38H1G1 (Limited Auto-Send Rollout Cap):** When automatic follow-up sending is first enabled, the rollout shall cap itself at 10 successful follow-up sends, then pause follow-up auto-send and surface the results for inspection before sending more.
...
- **FR-SYS-38H1G1C (No Post-Cap Dry-Run Continuation Rule):** After the initial 10-send rollout cap is reached, the follow-up worker shall not continue evaluating later candidates in dry-run or inspection mode by default. It shall remain paused until the owner inspects the first 10 sends and explicitly resumes follow-up processing.
- **FR-SYS-38H1G2 (Post-Inspection Resume Policy Deferred):** After the initial 10-send rollout has been inspected, the follow-up worker shall remain paused until the owner explicitly chooses the next resume policy. The system shall not assume uncapped continuation or a second bounded batch by default. That post-inspection resume policy is intentionally owner-deferred and must be set when the owner is ready to continue.
- **FR-SYS-38H1G2A (Combined Initial Rollout Queue Rule):** The initial capped live rollout shall draw from one combined eligible follow-up queue that includes both historical eligible backlog and newly due follow-ups. The worker shall not reserve the first rollout only for newly due threads; it shall use the same oldest-original-email-first ordering across the combined queue.
...
- Notes:
  ...
  - The current default permits up to three follow-ups per original outreach thread, with valid automatic sequence values `1`, `2`, and `3`.
  - The persistence layer shall enforce uniqueness for (`original_outreach_message_id`, `followup_sequence`) so repeated worker cycles cannot create duplicate follow-up plans for the same original outreach email and same sequence step.
...
```json
{
  ...
  "active_intended_contact_count_before_apollo": 3,
  "active_apollo_manager_contact_count_before_apollo": 0,
  "apollo_added_contact_count": 2,
  "resolved_company": {
  ...
  "candidates": [
    ...
    },
    {
      ...
      "relevance_reason": "current-company leadership contact shortlisted for Apollo manager expansion",
      "has_email": true,
      "has_direct_phone": true,
...
- **FR-RT-12C5 (Failed Retailoring Run Does Not Reuse Prior Approved Output):**
  If a newer retailoring run for a posting ends in `tailoring_status = needs_revision` or `tailoring_status = failed` with `resume_review_status = not_ready`, runtime shall not keep using an older approved run for Outreach. The posting returns to `tailoring_in_progress` and remains set aside until a newer run reaches `resume_review_status = approved`.
...
- **FR-EO-00 (Contact Search Responsibility):** The Outreach-side discovery layer shall own seeded-contact enrichment and bounded Apollo company-scoped search for role-targeted leads, including manager-expansion leadership harvest where applicable.
...
2. Apollo enrichment plus bounded Apollo current-company search for manager-expansion leadership harvest
...
- **FR-ED-00A (Source-Seeded Discovery Entry):** For role-targeted postings, the discovery layer shall begin from the source-seeded contacts captured during lead ingestion and may use company-scoped Apollo search only when the posting is intentionally running the manager-expansion leadership harvest or later replacement selection.
...
- **FR-ED-00B2B (Apollo Company-Match Validation Rule):** The Apollo company-resolution path shall not accept or persist a resolved organization record unless that organization still matches the posting's company identity using the strongest available hints in this order: a reliable company domain when one exists, a parseable employer slug or company handle from the apply URL when the posting is hosted on an ATS domain, and the normalized posting company name. Aggregator hosts such as `jobright.ai` and generic ATS hosts such as `jobs.lever.co`, `boards.greenhouse.io`, and `jobs.ashbyhq.com` shall not be treated as the employer's own company domain.
- **FR-ED-00B2C (Invalid Resolved-Organization Rejection Rule):** If Apollo company search returns a top record whose normalized organization name and domain signals do not match the posting company identity under `FR-ED-00B2B`, the runtime shall reject that resolution, skip promotion of `provider_company_key`, and avoid materializing Apollo people-search candidates from that mismatched organization into the posting frontier.
- **FR-ED-00B2D (Persisted Apollo-Company Reuse Validation Rule):** Before reusing a previously persisted Apollo organization identifier from `provider_company_key`, saved provider context, or historical people-search artifacts, the runtime shall revalidate that organization against the current posting company identity under `FR-ED-00B2B`. A stale or mismatched persisted Apollo organization shall be treated as invalid rather than silently reused.
- **FR-ED-00B2E (Invalid Apollo-State Repair Rule):** When the runtime detects that a posting is already carrying a mismatched persisted Apollo organization or Apollo-added contacts from the wrong company, it shall repair that posting before continuing discovery by clearing the invalid company-resolution anchor and removing the mismatched unsent Apollo-added contacts from the intended outreach set so later cycles cannot continue drafting or sending against the wrong company.
- **FR-ED-00B2F (Ambiguous Weak-Identity Company Resolution Rule):** When reliable employer-domain evidence is unavailable and the posting identity collapses to a weak single normalized token such as a generic company name plus the same ATS-employer slug, the Apollo company-resolution path shall fail closed unless the resolved organization name itself normalizes to that exact token and the provider returns a unique best match. If multiple Apollo organizations satisfy that weak token-only identity, or the resolved organization introduces extra unmatched brand terms such as geography or another brand word, the runtime shall treat company resolution as ambiguous and skip Apollo people search rather than routing outreach to the wrong company.
...
- **FR-ED-00B6 (Job Posting Provider Context Table):** The main database shall include a `job_posting_provider_contexts` table for posting-scoped provider payloads such as Apollo company resolution. At minimum this table should support:
  ...
  2. `job_posting_id`
  3. `provider_name`
  4. `context_stage` such as `apollo_company_resolution`
  5. provider-scoped organization identifier when available
  6. full raw provider payload JSON
...
- **FR-ED-00D4 (Apollo Search-Payload Snapshot Rule):** For every shortlisted Apollo candidate that is materialized into canonical contact state, the system shall persist the full Apollo search-stage payload as a provider-profile snapshot linked to that `contact_id`. This snapshot shall preserve all Apollo fields returned for that candidate at search time, even when many of those fields are not yet used operationally.
...
- **FR-ED-00F (Apollo Manager-Harvest Breadth):** For the autonomous source-seeded mode, Apollo manager-expansion should be broad enough to collect relevant current-company leadership candidates but still bounded by the adaptive shortlist cap rather than unconstrained high-recall search.
...
- **FR-ED-09A (Transient Provider-Unavailability Is Not Contact Exhaustion):** Normalized provider outcomes that reflect temporary provider unavailability rather than contact truth, including `rate_limited`, `quota_exhausted`, `network_error`, and `provider_error`, shall not mark the contact or posting-contact link as exhausted for Email Discovery.
- **FR-ED-09B (Provider Cooldown Persistence Rule):** When Email Discovery receives a temporary provider-unavailability outcome, the system shall persist provider-scoped cooldown state and skip new calls to that provider until the cooldown expires. The current build shall use these defaults unless richer provider reset metadata is available: `15 minutes` for `rate_limited`, `network_error`, and `provider_error`; and `reset_at` when known or otherwise `24 hours` for `quota_exhausted`.
- **FR-ED-09C (Email-Discovery Runnable Gate):** A role-targeted `email_discovery` run shall be considered runnable only when at least one pending contact still lacks a usable email and at least one provider is currently callable for that contact. If all remaining pending contacts are blocked only by active provider cooldowns, the durable run shall remain at `email_discovery` but shall yield scheduler selection to other runnable work until a provider cooldown expires.
- **FR-ED-09C1 (Repeated Provider-Failure Exhaustion Rule):** Repeated non-success provider outcomes for the same posting-linked contact shall not leave that contact pending forever. When the same provider returns a bounded repeated series of non-usable outcomes that are not producing a usable email for that contact, the runtime shall treat that provider path as spent for exhaustion accounting on that contact rather than retrying it indefinitely as if it were always freshly actionable.
- **FR-ED-09C2 (Permanent Frontier Degradation Rule):** If the remaining pending contacts in the current selected send slice are no longer producing any callable or plausibly recoverable provider paths after bounded retries, the runtime shall mark those posting-contact links exhausted and allow the posting frontier to settle rather than looping forever in `email_discovery` on the same contacts.
- **FR-ED-09C3 (Stale Email-Discovery Run Settlement Rule):** A role-targeted durable run shall not remain indefinitely `in_progress` at `email_discovery` once the current auto-sendable frontier has settled into only cooldown-blocked contacts or only exhausted contacts. Cooldown-only frontiers may remain parked at `email_discovery` while yielding scheduler time. But if the remaining pending contacts no longer have any callable or recoverable provider path, the runtime shall exhaust those posting-contact links, reevaluate the send frontier, and then either advance to `sending` if a ready subset exists or close the stale `email_discovery` run into the appropriate downstream terminal state instead of leaving it stranded.
- **FR-ED-09C1 (Ready-Subset Progression Override Rule):** The runnable gate in `FR-ED-09C` shall not block a role-targeted `email_discovery` run from being selected when the current send slice already contains a ready subset that can advance the posting to `ready_for_outreach` or `sending`. In that case the supervisor shall allow one bounded `email_discovery` progression cycle to re-evaluate the frontier and promote the posting forward before yielding later cooldown-blocked contacts.
- **FR-ED-09D (Apollo-Settled Email-Discovery Suppression Rule):** A role-targeted posting whose bounded shortlisted contacts have already been Apollo-enriched and definitively settled with no usable emails shall not remain ordinary runnable `email_discovery` backlog. Once the bounded shortlist has been settled into either usable-email-ready or exhausted outcomes, the scheduler shall either advance to sending or park the posting/run with a clear Apollo-no-email exhaustion reason.
...
- **FR-ED-28E (Provider Snapshot Attachment Rule):** Full provider-specific person payloads shall not be squeezed into the core `contacts` columns. Instead, provider payloads such as Apollo search/enrichment responses shall attach through a dedicated provider-profile table linked by `contact_id`.
...
- **FR-ED-29A (Contact Provider Profile Table):** The main database shall include a `contact_provider_profiles` table for provider-specific person payload snapshots. At minimum this table should support:
  ...
  2. `contact_id`
  3. `provider_name`
  4. provider-scoped person identifier such as Apollo person ID
  5. provider-scoped organization identifier when available
  6. `profile_stage` such as `apollo_search` or `apollo_enrichment`
  7. full raw provider payload JSON
  8. provider refresh/observed timestamp when available
  9. `created_at`
  10. `updated_at`
- **FR-ED-29A1 (Raw Payload Fidelity Rule):** The raw provider payload stored in `contact_provider_profiles` and `job_posting_provider_contexts` shall preserve the exact provider response fragment or object returned by Apollo for that entity/stage, not a lossy normalized rewrite. Any normalized convenience fields should live in separate columns rather than replacing the raw payload.
- **FR-ED-29B (Contact Employment History Table):** The main database shall include a `contact_employment_history` table so drafting and review can query Apollo employment history in structured form. At minimum this table should support:
  1. `contact_employment_history_id`
  2. `contact_id`
  3. `provider_name`
  4. provider-scoped person identifier
  5. employer/company label
  6. role title
  7. start date when available
  8. end date when available
  9. current-role flag when available
  10. source sort/order index
  11. raw per-item payload JSON
  12. `created_at`
  13. `updated_at`
- **FR-ED-29C (Provider Snapshot Uniqueness and Indexing Rule):** The provider-snapshot tables shall support efficient lookup by `contact_id`, `job_posting_id`, `provider_name`, provider-scoped Apollo identifiers, and recency. The schema should enforce practical uniqueness for exact duplicate snapshot rows where feasible and should at minimum index:
  1. `contact_provider_profiles(contact_id, provider_name, created_at)`
  2. `contact_provider_profiles(provider_name, provider_person_id)`
...
- **FR-EM-01A (Current Subject Ownership Rule):** Drafting shall explicitly generate or assign a subject line as part of the outreach draft. In the current role-split build, deterministic Python owns the final subject and uses the fixed per-path patterns defined by the active drafting strategy.
...
- **FR-EM-01B4 (Failed-Draft Requeue Eligibility Rule):** A posting-contact pair that has only prior role-targeted draft-attempt rows in terminal `failed` state shall not be treated as permanently touched for future drafting eligibility. If that posting is later requeued after a repair or operator-directed retry and the contact is otherwise still ready, the drafting frontier may regenerate a fresh draft for that same posting-contact pair.
...
- **FR-EM-01D (Role-Targeted Draft Inputs):** For role-targeted outreach, draft generation shall explicitly use job-posting context, recipient profile context when available, the tailored resume or bounded sender evidence derived from it, and the current role-split input pack for the selected recipient path as core inputs.
...
- **FR-EM-03A2A (Non-Runnable Generated Draft Retirement Rule):** If a persisted role-targeted generated draft no longer belongs to the current runnable frontier because its posting-contact link has left the intended outreach set, its posting has already reached a terminal completed state, or its send frontier is otherwise no longer valid, the runtime shall retire that unsent draft into a non-resumable terminal state instead of leaving it indefinitely in `generated`.
...
- **FR-EM-03F00B (Manual Helper Exclusion Boundary):** The Codex-only removal requirement applies to scheduled or autonomous production workers and their mirrored dry-run or review rendering paths. Owner-invoked manual drafting helpers may remain outside this requirement until the owner explicitly asks to migrate or retire them.
- **FR-EM-03F00C (Production Reachability Removal Rule):** In `main`, retired deterministic or shared-template outreach renderers shall be removed from scheduled or autonomous production worker wiring entirely. If any legacy deterministic renderer code must remain temporarily for a manual-only helper, that code shall be isolated behind a manual-only entrypoint and must not remain reachable through supervisor, follow-up worker, dry-run worker, inspection rendering, or any other autonomous production path.
- **FR-EM-03F00D (Deterministic Backlog Auto-Retirement Rule):** When `main` is cut over to Codex-only scheduled or autonomous production outreach, any still-unsent production outreach backlog that was generated by the retired deterministic or shared-template renderer shall be retired automatically rather than left pending. Those deterministic-origin unsent drafts or blocked-send rows shall be moved into a non-resumable terminal state with a clear cutover reason preserved in canonical state, and the supervisor or follow-up worker shall not later resurrect them automatically.
- **FR-EM-03F00E (Deterministic-History Preservation Rule):** Auto-retiring old unsent deterministic production backlog shall preserve already-sent outreach history, existing audit artifacts, and review traceability. The cutover should prevent future autonomous use of that backlog without erasing the historical evidence of what was previously generated or sent.
- **FR-EM-03F00F (Deterministic-Origin Follow-Up Plan Retirement Rule):** During the same Codex-only cutover, any pending automatic follow-up plan whose original first email is deterministic-origin or otherwise excluded from the Codex-origin automatic follow-up scope shall be auto-skipped into a terminal non-sendable state with a structured cutover reason. The plan row and its audit history shall remain queryable, but the follow-up worker shall not continue treating that thread as actionable automatic follow-up backlog.
- **FR-EM-03F00G (Unknown-Origin Fail-Closed Rule):** During the Codex-only scheduled or autonomous production cutover, any production backlog item whose drafting origin cannot be proven to be valid under the active Codex-origin rules shall fail closed. The autonomous worker shall retire, skip, or otherwise remove that item from automatic sendability with a structured reason such as `unknown_origin_during_codex_cutover` rather than allowing it to continue based only on surface content quality.
- **FR-EM-03F00H (Codex-Outage Incident And Pause Rule):** If a scheduled or autonomous production outreach worker cannot initialize the required Codex renderer, cannot execute `codex exec`, or otherwise loses the ability to produce valid Codex-origin drafts for its production scope, the system shall create a clear incident and pause the smallest autonomous worker scope that prevents repeated retries or accidental sends. Worker-specific pause is preferred when supported, but a broader global autonomous pause is acceptable if no narrower control exists. In all cases, the system shall fail closed rather than falling back to retired deterministic or shared-template production drafting.
- **FR-EM-03F00H1 (Autonomous Codex Wall-Clock Timeout Rule):** Scheduled or autonomous production `codex exec` drafting calls shall use a bounded wall-clock timeout of 10 minutes. If the invocation exceeds that budget, the runtime shall terminate the attempt, persist any partial stdout/stderr artifacts for debugging, record the failed invocation in canonical LLM usage logs, and return control to the worker rather than allowing one slow draft to monopolize the supervisor or follow-up lane indefinitely.
- **FR-EM-03F00I (Surviving Codex Backlog Refresh Rule):** If an unsent autonomous production outreach item survives the Codex-only cutover because its origin is valid under the Codex rules, the worker shall refresh that pending draft from the current accepted Codex drafting logic before autonomous send when the normal stale-draft refresh gates apply. The cutover shall not assume that an older unsent Codex-origin draft is automatically safe to send unchanged.
- **FR-EM-03F00J (Manual-Only Legacy Isolation Rule):** Any legacy deterministic outreach code that remains temporarily for owner-invoked manual helpers shall be clearly isolated from autonomous worker wiring and operational controls. Autonomous runtime state, incidents, and review artifacts shall make it clear that those manual-only paths are outside the Codex-only production boundary.
- **FR-EM-03F0 (Send-Stage Recheck Demotion Rule):** If a role-targeted `sending` run is reevaluated and the next selected contact frontier no longer has any draftable ready contacts because those contacts still need usable emails, the system shall not attempt drafting from that stale `ready_for_outreach` state. It shall persist the posting back to `requires_contacts` and move the durable run back to `email_discovery`. That demotion path shall not require new `outreach_messages` rows or `send_result` artifacts for that cycle because no drafting or sending occurred.
...
- **FR-EM-03L0A1 (Per-Original-Email Follow-Up Scope):** If the same contact has multiple separate original `role_targeted` outreach emails for different roles or distinct Gmail threads, each original email may be eligible for its own follow-up sequence. Duplicate suppression shall operate per original outreach email and Gmail thread, not by contact alone, unless thread evidence shows the messages belong to the same conversation.
- **FR-EM-03L0A1A (One Active Follow-Up Thread Per Contact):** When the same contact has multiple separate eligible original outreach threads, the follow-up worker shall allow only one active automatic follow-up sequence for that contact at a time. The default tie-breaker shall prefer the most recent original outreach thread by `original.sent_at DESC`. Older sibling threads for the same contact shall be skipped or held with a structured reason until the newer thread is exhausted, replied to, bounced, or otherwise leaves the automatic follow-up queue.
- **FR-EM-03L0A1B (Cross-Contact Independence Rule):** Automatic follow-up suppression shall not widen from one contact to other contacts at the same company or team by default. Aside from explicit reply-driven or do-not-contact contact-wide stops, follow-up eligibility is evaluated per contact and per original thread rather than being blocked by other unreplied threads at the same employer.
...
- **FR-EM-03L0B1 (Posting Lifecycle Non-Blocking Rule):** Once an original outreach thread exists, later job-posting lifecycle changes such as archived, closed, removed, or otherwise unavailable posting state shall not by themselves block the next allowed follow-up. Follow-up eligibility remains email-thread-rooted as long as the original thread, reply/bounce checks, and required drafting evidence are still valid.
...
- **FR-EM-03L1A (Autonomous First Email Scope):** Automatic follow-up candidates shall be limited to first emails sent autonomously by the system and persisted as original sent `role_targeted` outreach messages in canonical state. First emails that were manually triggered by the owner, even if recorded through the system, shall remain owner-managed and shall not enter the automatic follow-up worker by default. The follow-up worker shall not attempt to follow up on arbitrary external Gmail messages that were never recorded as autonomous system outreach.
- **FR-EM-03L1B (Codex-Origin Original-Email Scope):** Automatic follow-up candidates shall be limited further to autonomous original `role_targeted` emails whose first-email draft was produced by the newer Codex role-split production pipeline. Older autonomous original emails sent from the retired deterministic/shared first-email renderer shall not enter the automatic follow-up worker by default.
- **FR-EM-03L1B0 (Codex-Origin Evidence Precedence Rule):** When determining whether an autonomous original `role_targeted` email came from the newer Codex role-split production pipeline, the worker shall prefer stronger provenance evidence first. The precedence shall be:
  1. explicit persisted origin metadata
  2. nearby local artifact evidence such as draft artifact paths, renderer/debug payloads, or other role-split artifact markers
  3. original sent email body/subject only as the last fallback
- **FR-EM-03L1B1 (Codex-Style Body Fallback Rule):** If older autonomous original `role_targeted` emails do not carry enough persisted origin metadata to prove whether the first email came from the newer Codex role-split pipeline or the retired deterministic/shared renderer, the follow-up worker may fall back to classifying the original body itself. This fallback shall require a strong/high-confidence match to one of the two known production Codex first-email families only:
  1. the technical-path `Learning from your career path` family
  2. the managerial-path `Interest in the <Role> role at <Company>` family
  Small non-semantic variation such as punctuation, whitespace, or role/company slot values may be tolerated, but broader style similarity is not enough. If the match is ambiguous, low-confidence, or closer to the retired deterministic/shared style, the worker shall exclude that thread from automatic follow-up by default.
- **FR-EM-03L1B2 (Historical Non-Codex Exclusion Handling):** When a historical autonomous original thread is excluded because it is deterministic-origin, clearly non-Codex in body shape, or only a low-confidence Codex-style match, the worker shall skip it with a structured suppression reason. It shall not escalate those exclusions into review by default.
- **FR-EM-03L1B3 (Durable Skip Record For Historical Non-Codex Exclusions):** When the worker evaluates a historical autonomous original thread and excludes it under the Codex-origin rules, it should still persist a durable skipped follow-up-plan record or equivalent audit row with a structured reason such as `non_codex_original`, `low_confidence_codex_origin`, or equivalent. Permanently excluded historical threads should remain auditable rather than disappearing silently.
...
- **FR-EM-03L4 (Continuous Follow-Up Plan Materialization):** The follow-up worker may create or refresh follow-up plans for sent `role_targeted` messages before they are due. Plans shall become sendable only after the applicable business-day follow-up eligibility threshold for that sequence step and all stop-condition, reply-thread, duplicate-follow-up, pacing, and agent-review gates pass.
- **FR-EM-03L4B (Next-Step-Only Plan Materialization):** The follow-up worker shall materialize only the next allowed follow-up step when it becomes relevant. It shall not pre-create all three possible follow-up sequence rows upfront for a thread. Follow-up `2` becomes materializable only after follow-up `1` has been sent and the `4` business-day timer becomes relevant; follow-up `3` becomes materializable only after follow-up `2` has been sent and the `5` business-day timer becomes relevant.
- **FR-EM-03L4A (No Early Follow-Up Draft Rendering):** The follow-up worker shall not render follow-up draft bodies or draft artifacts before the original email or prior follow-up reaches the applicable business-day eligibility threshold for the next sequence step. Pre-eligibility materialization may track candidate status and due time, but content generation shall wait until the next follow-up is eligible so stale drafts are not created.
- **FR-EM-03L4A1 (Due-Only Draft Artifact Persistence):** Drafted-but-unsent follow-up artifacts shall be rendered and persisted only when the candidate is actually due for that follow-up step. The worker shall not pre-render `due soon` follow-ups for speed.
- **FR-EM-03L4C (Historical Sequence Materialization Coverage Rule):** Historical follow-up plan materialization shall not depend solely on a recent-only sent-email slice that can starve older in-scope Codex-origin threads. The worker shall continue evaluating all in-scope Codex-origin originals that already have follow-up history or otherwise remain sequence-eligible, even when those originals are older than the newest recent send window.
- **FR-EM-03L5 (Historical Backfill Scope):** The follow-up worker shall consider historical autonomous original sent `role_targeted` emails from before the feature was implemented, not only future sends, but only when those original first emails came from the newer Codex role-split production pipeline. Historical candidates remain subject to the same autonomous-first-email scope, Codex-origin scope, duplicate-follow-up, reply, bounce, thread, grounding, and pacing gates as future candidates.
...
- **FR-EM-03L5C (Pre-Cutover Archive Skip Repair Rule):** A historical Codex-origin thread shall not remain permanently suppressed only because an earlier posting-archive cleanup marked its follow-up plan with `posting_archived_pre_cutover`. Because automatic follow-up eligibility is email-thread-rooted, the worker shall reopen those Codex-origin plans into the normal automatic queue unless another active terminal stop such as bounce, reply, duplicate follow-up evidence, multi-recipient scope, or newer-thread preference still applies.
- **FR-EM-03M (Current Follow-Up Cadence):** For unreplied sent outreach, the system shall support the original email plus up to three automatic follow-ups per original outreach thread. The current default cadence is:
  1. follow-up `1` becomes eligible after `3` business days from the original sent email
  2. follow-up `2` becomes eligible after `4` business days from follow-up `1`
  3. follow-up `3` becomes eligible after `5` business days from follow-up `2`
  No automatic follow-up `4` or later sequence is allowed under the current design.
- **FR-EM-03M0S (Follow-Up Timer Start):** Each follow-up timer shall start from the prior successful touch in the same thread. Follow-up `1` starts from the original outreach message's `sent_at`; follow-up `2` starts from follow-up `1`'s `sent_at`; follow-up `3` starts from follow-up `2`'s `sent_at`. Delivery Feedback states such as `not_bounced` shall be independent gates and shall not reset or delay the cadence timer.
- **FR-EM-03M0TZ (Follow-Up Timezone):** Follow-up business-day eligibility shall be calculated in the owner's local timezone, currently `America/Phoenix`, so the worker's due-date behavior matches how Gmail sent times appear to the owner.
- **FR-EM-03M0TZ1 (Business-Day Definition):** For the current follow-up cadence, `business day` means weekdays only, Monday through Friday, in the owner's local timezone. The worker shall not exclude U.S. holidays or other holiday calendars unless the owner explicitly adds holiday-aware follow-up scheduling later.
- **FR-EM-03M0TZ2 (Cadence Days Versus Send Windows Separation Rule):** The follow-up `3 / 4 / 5` cadence continues to use business-day math even though the shared outbound send-window policy runs all 7 days. Business-day cadence determines when a follow-up becomes due; the 24-hour shared send-window policy determines when a due follow-up may actually take a send turn.
- **FR-EM-03M0TZ3 (Weekend Send Allowed After Due Rule):** Once a follow-up has become due under the business-day cadence, it may use any eligible shared send window on Saturday or Sunday as well. Weekend exclusion applies only to cadence counting, not to final send execution after the thread is already due.
- **FR-EM-03M0TZ4 (Existing Unsent Plan Cadence Refresh Rule):** When the configured follow-up cadence changes, the worker shall recalculate `eligible_after` for all unsent automatic follow-up plans from the same per-sequence business-day rules used for new plans. Sent follow-up history remains immutable, but existing `pending`, `held_for_review`, or retryable unsent plans shall not keep stale later dates from the retired cadence.
- **FR-EM-03M0TZ5 (Due-Immediately After Refresh Rule):** If refreshing an unsent follow-up plan under the current cadence yields an `eligible_after` that is already in the past, that plan becomes immediately due. The system shall not preserve the older later due date solely because the plan was materialized under a previous cadence.
- **FR-EM-03M0TZ6 (Local Time Preservation Rule):** Follow-up cadence refresh and new due-date calculation shall preserve the prior touch's local `America/Phoenix` send time-of-day. The worker changes only the business-day gap, not the local clock time anchor.
- **FR-EM-03M0 (No Maximum Follow-Up Age Cutoff):** The current design shall not impose a maximum original-email age cutoff for automatic follow-ups. An older sent email may still receive its next allowed follow-up if it is otherwise eligible and all gates pass.
- **FR-EM-03M0A0 (No Historical Step Jumping Rule):** When an older eligible original outreach thread has aged far past the normal cadence, the worker shall still begin with follow-up `1`. It shall not jump directly to follow-up `2` or follow-up `3` solely because more calendar time has elapsed since the original email. Later steps become eligible only after the prior follow-up in that same thread has actually been sent and its own `4` or `5` business-day timer has then elapsed.
- **FR-EM-03M0A0A (No Extra Subjective Delay Buffer):** The configured `3 / 4 / 5` business-day cadence is the full timing buffer for automatic follow-ups. Once a thread crosses the applicable cadence threshold and is inside the currently allowed shared send window, it is eligible; the worker shall not apply an extra subjective `too recent` delay layer beyond the configured cadence.
- **FR-EM-03M0A0B (No Weekend-Adjacency Special Case):** The follow-up worker shall not apply special timing treatment just because the original email was sent near a weekend. Business-day counting alone governs the due date, so a Friday original may yield a Thursday follow-up `1` exactly by the normal cadence math.
- **FR-EM-03M0A0C (Original Send-Time Non-Blocking Rule):** An original first email sent outside any older preferred business-hours window shall not by itself block a later automatic follow-up. If the follow-up is otherwise due and the follow-up send itself occurs inside an eligible shared send window, the thread remains eligible.
- **FR-EM-03M0A (Full-Day Shared Send-Window Policy):** Automatic role-targeted outreach and automatic follow-up sending shall use the full 24-hour day rather than a reduced `5:00 AM - 5:00 PM` send window. The current autonomous send-window policy shall be evaluated in `America/Phoenix` and shall run all 7 days of the week.
- **FR-EM-03M0A1 (Alternating Two-Hour Send Windows):** The shared outbound lane shall alternate preferred send type every 2 hours across the full local day. The current cycle shall start with a follow-up-preferred window at local midnight, then alternate every 2 hours:
  1. `12:00 AM - 1:59 AM`: follow-ups preferred
  2. `2:00 AM - 3:59 AM`: original role-targeted outreach preferred
  3. `4:00 AM - 5:59 AM`: follow-ups preferred
  4. `6:00 AM - 7:59 AM`: original role-targeted outreach preferred
  5. `8:00 AM - 9:59 AM`: follow-ups preferred
  6. `10:00 AM - 11:59 AM`: original role-targeted outreach preferred
  7. `12:00 PM - 1:59 PM`: follow-ups preferred
  8. `2:00 PM - 3:59 PM`: original role-targeted outreach preferred
  9. `4:00 PM - 5:59 PM`: follow-ups preferred
  10. `6:00 PM - 7:59 PM`: original role-targeted outreach preferred
  11. `8:00 PM - 9:59 PM`: follow-ups preferred
  12. `10:00 PM - 11:59 PM`: original role-targeted outreach preferred
- **FR-EM-03M0A2 (Cross-Window Fallback Capacity Rule):** The preferred send type for a 2-hour window shall not waste capacity. If a follow-up-preferred window has no due follow-ups ready to send, original role-targeted outreach may use that window. If an original-outreach-preferred window has no ready original role-targeted outreach, due follow-ups may use that window.
- **FR-EM-03M0A2A (Fallback Uses Current Queue Reality Rule):** Cross-window fallback shall use the actual queue state at the moment of final send attempt. If a draft was prepared earlier for one queue but the currently preferred queue is empty when the shared pacing gate opens, the other queue may use that send turn under the fallback rule rather than waiting for its next preferred window.
- **FR-EM-03M0A2B (Unsafely-Blocked Preferred Queue Does Not Hold The Lane):** If the preferred queue for the active window has candidates nominally present but none of them is actually sendable at the final send attempt because of a last-second safety gate such as reply detection, bounce evidence, missing thread context, or equivalent immediate stop conditions, that queue shall be treated as effectively empty for that send turn. The other queue may use the turn under the fallback rule if it has a currently sendable candidate.
- **FR-EM-03M0A2C (Sendable-Now Queue Evaluation Rule):** The window router shall decide fallback and queue preference using candidates that are actually sendable now, not merely drafted, nearly ready, or waiting behind some additional internal step. Near-ready or drafted-but-not-sendable items shall not hold the shared lane or block the other queue from using a send turn.
- **FR-EM-03M0A2C1 (Stale Drafts Are Not Sendable-Now):** If a queued draft has crossed the freshness threshold and must be refreshed before send, that candidate shall no longer count as `sendable now` for window ownership or fallback purposes until the refresh succeeds.
- **FR-EM-03M0A2D (Active Window Preference Still Wins Rule):** If both queues each have at least one `sendable now` candidate, the active 2-hour window preference shall still decide which queue receives the turn. Cross-window fallback only activates when the currently preferred queue is effectively empty for that send turn.
- **FR-EM-03M0A2E (Fallback Owns The Window Until Preferred Work Appears):** When fallback activates because the preferred queue for the current 2-hour window is effectively empty, the non-preferred queue may continue using that same window for additional send turns as long as it remains sendable and the preferred queue still has no sendable-now candidate. The worker shall not artificially stop after one fallback send if the window still has usable capacity.
- **FR-EM-03M0A2F (Preferred Queue Reclaims The Window On The Next Turn):** If fallback has been using a window and the preferred queue becomes sendable again before the 2-hour block ends, the preferred queue shall reclaim the lane on the next eligible send turn after the shared pacing gate opens. Fallback does not permanently take ownership of the rest of that window.
- **FR-EM-03M0B (Follow-Up Send Pacing):** Follow-up sends shall use the follow-up cadence as the primary timing control, but once a follow-up is due it shall obey the same randomized `6-10 minute` inter-send gap used by normal initial outreach so outbound email does not burst.
- **FR-EM-03M0B1 (Shared Outbound Send Lane):** Initial outreach sends and follow-up sends shall share one synchronized outbound pacing lane. The supervisor and the dedicated follow-up worker shall not send concurrently or bypass each other; all automatic outbound emails must respect one serialized global send order so sends occur one after another rather than colliding.
- **FR-EM-03M0B1A (Window-Routed Shared Lane Rule):** When both new original outreach and due follow-ups exist, the shared outbound lane shall route the next send according to the active 2-hour local send window rather than giving one queue permanent global priority. Original or follow-up sends shall enter `waiting_for_pacing` when they are otherwise ready but the active window currently prefers the other queue and that preferred queue has actionable work.
- **FR-EM-03M0B1A5 (Distinct Window-Wait Reason Rule):** The runtime may keep the existing durable `waiting_for_pacing` plan status for sendable candidates deferred by the 2-hour window router, but it shall persist a distinct structured wait reason for `waiting_for_window` versus the older `waiting_for_pacing_gap` / inter-send-gap case. This preserves current dashboards and plan-state shapes while making it clear whether a candidate is waiting because of the active window policy or because the global `6-10 minute` gap has not opened yet.
- **FR-EM-03M0B1A5A (Window-Wait Requires Otherwise-Sendable State):** A candidate shall be marked `waiting_for_window` only when it is otherwise fully sendable for the current turn and the active 2-hour window prefers the other queue. Candidates still missing a valid draft, still blocked by reply/bounce/safety checks, still requiring stale-draft refresh, or otherwise not actually sendable shall use their own more specific states rather than being flattened into `waiting_for_window`.
- **FR-EM-03M0B1A6 (No Special Boundary Wake-Up Rule):** Candidates waiting only because the wrong 2-hour window is active do not require a special immediate wake-up exactly at the boundary. They shall be reconsidered on the next normal worker cycle after the active local window changes.
- **FR-EM-03M0B1A7 (Window-Queued Draft Freshness Rule):** A draft prepared in one window may be reused in a later send window if it is still recent and no material thread, reply, bounce, or safety facts changed. If the queued unsent draft is more than 24 hours old at actual send-attempt time, the worker should refresh the draft before send rather than relying on a stale queued body.
- **FR-EM-03M0B1A7D (Shared Freshness Rule Across Both Queues):** The queued-draft freshness policy applies to both original-outreach drafts and follow-up drafts that are waiting in the shared outbound lane. The runtime shall not refresh only one queue while allowing the other queue to send arbitrarily stale prepared drafts by default.
- **FR-EM-03M0B1A7A (Stale Draft Refresh Keeps Its Slot First):** When a queued draft crosses the freshness threshold but remains otherwise eligible, the worker should try to refresh that same candidate in place rather than immediately discarding its ready-queue slot. Only if refresh fails or the candidate is no longer eligible should that slot be freed for a different candidate.
- **FR-EM-03M0B1A7B (Successful Refresh Resets Freshness Age):** When a stale queued draft is refreshed successfully, that refresh timestamp becomes the new freshness reference for the queued draft. The worker shall evaluate the 24-hour freshness threshold from the latest successful draft materialization, not from the older superseded draft timestamp.
- **FR-EM-03M0B1A7C (Refresh Failure Uses The Same Draft-Retry Budget):** A stale-draft refresh failure shall use the same existing drafting retry budget for that queued candidate rather than creating a separate refresh-only retry allowance. Refresh is still a drafting attempt on the same candidate, so its failure semantics remain part of the normal bounded drafting retry policy.
- **FR-EM-03M0B1A1 (Window Policy Applies To Final Send Execution Only):** The 2-hour alternating send-window policy shall govern final outbound send execution only. The supervisor and follow-up worker may still perform safe preparatory work such as eligibility evaluation, reply guards, and draft rendering outside the preferred send window, but the actual Gmail send decision must respect the active local send window.
- **FR-EM-03M0B1A1A (Bounded Pre-Send Preparation Rule):** Because send windows govern final send execution only, workers may perform draft preparation outside the preferred window, but that preparation shall remain bounded. The system shall not pre-draft an unbounded backlog merely because send is deferred; it should maintain only a modest ready queue so stale drafts and unnecessary LLM usage do not accumulate.
- **FR-EM-03M0B1A1B (Ready-Queue Size Cap):** The current bounded-preparation default shall cap each queue's prepared-but-unsent ready set at 10 drafts at a time. The runtime may refresh, consume, or replace those drafts as sends happen, but it shall not keep building an unlimited prepared backlog on either the original-outreach side or the follow-up side.
- **FR-EM-03M0B1A1B0 (Legacy Over-Cap Backlog Is Not Auto-Retired):** If the system already holds more than 10 prepared-but-unsent drafts in a queue at the moment this policy is introduced, it shall not auto-delete or auto-retire that existing backlog solely because it exceeds the new cap. Instead, the cap governs the actively maintained prepared frontier going forward, while older persisted backlog outside that frontier remains dormant until it is refreshed, consumed, or otherwise resolved by normal queue rules.
- **FR-EM-03M0B1A1B0A (Active Frontier Uses Queue Ordering):** When a queue contains more persisted prepared drafts than fit inside the active frontier cap, the active frontier shall be the top-ranked subset under that queue's normal ordering rules. Lower-ranked persisted drafts remain dormant until a frontier slot opens or a later reevaluation changes their relative priority.
- **FR-EM-03M0B1A1B0B (Dormant Drafts Are Not Sendable-Now):** Persisted drafts that sit outside the active frontier cap shall not count as `sendable now` for active-window ownership, cross-window fallback, or ready-cap fullness decisions merely because they already exist in storage. Only candidates currently inside the bounded active frontier and passing the ordinary freshness/safety checks participate in live send-lane routing.
- **FR-EM-03M0B1A1B0C (Original Frontier Uses Current Posting Frontier Only):** For original role-targeted outreach, the bounded active prepared frontier shall be built from each posting's current generated send frontier rather than from every historical generated row for that posting. A posting with many persisted generated drafts shall contribute only its current next generated send candidate to the active original frontier until that candidate is sent, invalidated, or otherwise leaves the frontier, after which the next generated candidate from that same posting may enter by normal ordering.
- **FR-EM-03M0B1A1B0D (Daily-Cap-Blocked Original Drafts Become Dormant):** A generated original draft whose posting cannot take another original send in the current local day because the posting-level daily send cap is already exhausted shall not consume an active original prepared-frontier slot during that blocked period. Those generated drafts remain persisted and auditable, but they become dormant for frontier-cap accounting and active routing until the posting regains same-day send capacity.
- **FR-EM-03M0B1A1B0E (Dormant Draft Reentry Requires Normal Recheck):** When a dormant persisted draft later rises back into the active frontier because a slot opens or reevaluation changes priority, it shall reenter the live queue only after the normal freshness, reply/bounce, and other send-safety checks are re-evaluated. Dormant status does not grandfather a stale or otherwise invalid draft into immediate sendability.
- **FR-EM-03M0B1A1B1 (Shared Follow-Up Ready Cap Across Sequences):** The follow-up-side ready cap is shared across the whole follow-up queue rather than being partitioned into separate caps for follow-up `1`, `2`, and `3`. Due-time ordering and sequence tie-break rules decide which follow-up drafts occupy those 10 ready slots.
- **FR-EM-03M0B1A1B2 (Ready-Cap Admission Uses The Same Queue Ordering):** When more candidates are due or send-ready than fit inside the prepared-draft cap, admission into the ready set shall use the same queue-ordering rules that govern sending. For follow-ups, the ready set shall admit the oldest-due candidates first with the configured sequence tie-breakers. For original outreach, the ready set shall admit the oldest generated/oldest-ready candidates first.
- **FR-EM-03M0B1A1B3 (No Immediate Fresh-Draft Eviction For A Newcomer):** If a newly eligible candidate outranks the lowest draft currently inside the active frontier, the runtime should not immediately evict a still-fresh and otherwise valid queued draft just to reshuffle the frontier. Frontier turnover should happen naturally as slots are consumed, refreshed, invalidated, or otherwise freed, unless a stronger explicit displacement rule is introduced later.
- **FR-EM-03M0B1A1C (Ready-Queue Cap Includes Window-Waiting Drafts):** The ready-queue size cap applies to all prepared-but-unsent drafts in that queue, including drafts currently waiting only because of the active window or the shared pacing gate. The cap is meant to bound total prepared backlog, not just the subset that is immediately sendable.
- **FR-EM-03M0B1A1D (Preparation Effort Shifts Away From A Full Queue):** If one queue has already reached its prepared-draft cap, workers shall stop pre-drafting additional items for that same queue until capacity is consumed or freed. Preparation effort should shift to the other queue or to upstream eligibility work rather than overfilling one side of the shared lane.
- **FR-EM-03M0B1A1D1 (Freed Frontier Slots May Refill In The Same Bounded Cycle):** If a prepared draft leaves the active frontier during a worker cycle because it was sent, suppressed, blocked, refreshed into a new version, or otherwise invalidated, the worker may refill that newly freed frontier slot in the same cycle as long as it remains within the cycle's normal bounded evaluation budget.
- **FR-EM-03M0B1A1D2 (Prepared-Frontier Cap Is Global Across Workers):** The prepared-draft cap applies to the canonical queue as a whole, not separately per worker process, machine-thread, or cycle-local cache. The supervisor, follow-up worker, and any related send-preparation path shall observe one durable global prepared-frontier count per queue so parallel workers cannot each prepare up to the cap independently.
- **FR-EM-03M0B1A1D3 (Paused Or Disabled Queue Is Effectively Empty For Routing):** If one queue is currently paused, rollout-capped, disabled, or otherwise not allowed to send in the current runtime state, that queue shall be treated as effectively empty for active-window routing and cross-window fallback purposes until it becomes send-enabled again. The active window shall not reserve send turns for a queue that is administratively unable to use them.
- **FR-EM-03M0B1A1D4 (Paused Queue Preserves Dormant Prepared Drafts):** Treating a paused or disabled queue as effectively empty for routing does not require deleting its already-prepared drafts. Those drafts may remain dormant and reusable subject to the normal freshness and safety rules once that queue is resumed.
- **FR-EM-03M0B1A1E (Preparation Favors The Currently Active Window First):** When both queues still have room under their prepared-draft caps, preparation should favor the queue preferred by the currently active local 2-hour window first. If that preferred queue has no additional eligible candidate to prepare, workers may use the remaining preparation budget on the other queue or on upstream work.
- **FR-EM-03M0B1A1E1 (Active-Window Preparation Preference Is Not Total Monopolization):** Favoring the currently active window's queue for preparation does not require consuming all remaining bounded preparation budget on that queue exclusively. If the preferred queue already has a healthy prepared frontier or no urgent additional candidate to prepare, workers may use leftover bounded preparation budget on the other queue so the next window is not starved.
- **FR-EM-03M0B1A1E2 (Minimum Cross-Queue Readiness Floor):** If both queues have eligible candidates, both are below their prepared-frontier caps, and one queue currently has no fresh prepared draft in its active frontier, workers should use enough leftover bounded preparation budget to prepare at least one fresh candidate for that otherwise-empty queue when feasible. Active-window preference still applies first, but it shall not leave the other queue entirely cold when a low-cost readiness floor can be established safely.
- **FR-EM-03M0B1A1F (No Immediate Re-Draft Churn Within The Same Freshness Window):** If a queued draft remains under the 24-hour freshness threshold and no material thread, reply, bounce, or safety facts changed, the runtime should not keep re-drafting that same candidate on every worker cycle just because the queue is being re-evaluated. Freshness-based refresh should happen only when the threshold is crossed or a material send-safety/input fact changes.
- **FR-EM-03M0B1A1F0 (Stale Queued Draft Still Occupies Its Frontier Slot):** If a queued draft crosses the freshness threshold and now requires refresh before send, that candidate still occupies its active-frontier slot until the runtime refreshes it successfully, invalidates it, or removes it through normal queue turnover. The worker shall not temporarily treat stale queued drafts as cap-free placeholders and overfill the frontier with extra new drafts.
- **FR-EM-03M0B1A1F1 (Refresh Existing Frontier Candidate Before Lower-Priority Replacement):** If a stale queued draft still ranks inside the active frontier and remains otherwise eligible, workers should attempt to refresh that existing candidate before using the same bounded frontier capacity on a brand-new lower-priority candidate. Only if the stale candidate fails refresh, loses eligibility, or falls out of frontier ranking by normal reevaluation should that slot be spent elsewhere.
- **FR-EM-03M0B1A1F2 (Stale Original Frontier Remains Refresh-Actionable):** A stale original role-targeted draft inside the active original prepared frontier shall remain actionable for supervisor refresh/re-render work even though it is not `sendable now` for active-window ownership or cross-window fallback. The runtime must not let stale frontier drafts make the original queue appear completely non-actionable; otherwise the supervisor can skip the very sending-stage work required to refresh those drafts and unblock later original sends.
- **FR-EM-03M0B1A2 (Send-Attempt-Time Window Evaluation Rule):** Window eligibility shall be evaluated at the actual send-attempt time, not at the earlier draft-ready time. A draft prepared during one preferred window shall not bypass the routing policy later if the final Gmail send is attempted in a different local 2-hour window.
- **FR-EM-03M0B1A2A (Authoritative Final Queue Recheck Rule):** The queue that actually receives a send turn shall be chosen from authoritative canonical queue state at the moment the worker acquires the shared outbound lane and is ready to attempt send. Earlier cycle-local snapshots may guide preparation, but they are provisional only. The final routing decision must recheck the active local window, fallback condition, pacing gate, and sendable-now frontier against current canonical state immediately before send.
- **FR-EM-03M0B1A2B (No Cross-Queue Worker Impersonation Rule):** The shared-lane routing policy does not require every worker to be able to send every queue type. If a worker acquires the shared outbound lane and the authoritative final queue recheck shows that the current send turn belongs to the other queue family, that worker shall not attempt to impersonate the other queue's send path. It shall release the lane cleanly and leave the actual send to the appropriate worker on the next normal cycle.
- **FR-EM-03M0B1A2C (Yielding The Wrong Turn Does Not Consume Pacing):** When a worker acquires the shared outbound lane but yields because the authoritative final queue recheck says the turn belongs to the other queue, that no-send yield shall not advance the shared `last_send_at`, shall not consume the inter-send pacing gap, and shall not count as a send attempt for caps or rollout counters. It is only a routing handoff, not an outbound send event.
- **FR-EM-03M0B1A3 (Exact Boundary Evaluation Rule):** The active send window shall be determined by the exact local clock time at the moment of the final send attempt. The routing policy shall not apply a hidden grace buffer before or after the 2-hour boundary; a send attempt at the boundary uses the newly active window immediately.
- **FR-EM-03M0B1A3A (In-Flight Send Completion Rule):** If a final send attempt starts under a valid window decision and the 2-hour boundary flips while the Gmail send is already in flight, that in-flight send shall be allowed to complete. The window policy governs send-attempt start time, not mid-flight interruption.
- **FR-EM-03M0B1A4 (Global Pacing Still Wins Rule):** The 2-hour window router shall not bypass the single global `6-10 minute` inter-send pacing gate. If the preferred queue for the current window has actionable work but the global pacing gap is still closed, the other queue shall not jump ahead early. Both queues wait until the shared pacing gate opens, and the window router then decides which queue receives the next send turn.
- **FR-EM-03M0B1A8 (No Retroactive Window Catch-Up Rule):** If the machine sleeps, the runtime is paused, or workers otherwise miss one or more send windows, the system shall resume from the currently active local 2-hour window only. It shall not attempt to replay or catch up missed windows retroactively; overdue work is handled by the normal queue ordering rules.
- **FR-EM-03M0B1B (Original Queue Ordering Within Original Windows):** Inside an original-outreach-preferred window, ready original outreach shall be ordered by oldest generated draft first so already-drafted original emails do not starve behind newer send-ready originals.
- **FR-EM-03M0B1B1 (Deterministic Original Tie Ordering):** If multiple original drafts are tied on generated-draft age, the worker shall prefer the older posting/original thread context first, and if still tied shall fall back to stable canonical row ordering.
- **FR-EM-03M0C (No Follow-Up Company Or Posting Cap):** Follow-up sends shall not apply the normal per-posting, per-company, recipient-wave, or daily throughput caps from initial outreach. Once a sent outreach thread reaches the applicable business-day eligibility threshold for its next allowed follow-up and passes all stop-condition and agent-review gates, it is eligible for follow-up sending subject to the shared outbound pacing lane and the follow-up-specific timing rules.
- **FR-EM-03M0C1 (Original Send Caps Remain Unchanged Under Window Routing):** The new 24-hour alternating send-window policy shall not by itself change the current original role-targeted per-posting or other existing original-send caps. The first change in this policy family is lane routing and queue ordering only; original-send throughput caps remain as currently defined until explicitly revised later.
- **FR-EM-03M0D (Due Follow-Up Ordering):** When multiple follow-ups are due, the follow-up worker shall process them oldest due first, using `eligible_after ASC` as the primary ordering key rather than always preferring newly created follow-up `1` rows.
- **FR-EM-03M0D0 (Follow-Up Sequence Tie-Breaker Rule):** If multiple due follow-ups are tied or effectively equal on due-time ordering, higher follow-up sequence shall win before lower sequence, in the order `follow-up 3` before `follow-up 2` before `follow-up 1`. This prevents a flood of new first follow-ups from starving later follow-up steps on older threads.
- **FR-EM-03M0D0A (Deterministic Follow-Up Tie Ordering):** If due follow-ups are still tied after due-time ordering and sequence priority, the worker shall prefer the older original thread using `original.sent_at ASC`, and if still tied shall fall back to stable canonical row ordering. This keeps the queue deterministic and historically fair.
...
- **FR-EM-03M0E (Multiple Due Follow-Ups Allowed):** The follow-up worker may send multiple due follow-ups in the same local day. It shall continue working through due follow-ups in due-time order as long as each send still satisfies the shared outbound pacing gate, the active 2-hour local window policy, and all other follow-up safety rules.
- **FR-EM-03M1 (Three Follow-Up Default):** The current default follow-up sequence shall allow at most three follow-ups per original outreach thread. After follow-up `3`, the worker shall hard-stop and shall not auto-create or auto-send follow-up `4` or later.
- **FR-EM-03M2 (Approved Unreplied Follow-Up Drafting Strategy):** For unreplied sent outreach, the production follow-up drafter shall be Codex-based and sequence-aware rather than a single deterministic template body. It shall render concise same-thread follow-ups grounded in the original outreach thread, job context, and approved resume evidence while preserving the approved warm, professional tone.
- **FR-EM-03M2A (Sequence-Specific Follow-Up Intent):** The Codex follow-up drafter shall differentiate the three allowed follow-up steps:
  1. follow-up `1`: brief reminder / light bump
  2. follow-up `2`: slightly stronger mutual-fit framing without becoming long or pushy
  3. follow-up `3`: respectful final attempt that clearly implies no further follow-ups after this message
- **FR-EM-03M2A0 (Original Posture Continuity Rule):** Follow-up drafting shall preserve the original outreach posture rather than switching categories later in the sequence. Technical-path originals shall remain career-guidance / learning-oriented across all follow-ups, and managerial-path originals shall remain role-interest-oriented. Later follow-ups may become slightly stronger within that same posture, but they shall not turn a guidance thread into a direct application note or vice versa.
- **FR-EM-03M2A0B (Technical Follow-Up Hook Restraint Rule):** For technical-path originals, follow-ups may make only a light callback to the recipient's career path or work when that helps maintain thread continuity. They should otherwise focus on the reminder and guidance ask rather than reopening the full original career-path hook in later follow-ups.
- **FR-EM-03M2A0A (Managerial Role Anchor With Softer Later Framing):** For managerial-path originals, the specific role opening shall remain the anchor across the follow-up sequence. Follow-up `2` and follow-up `3` may soften into language about hearing the recipient's perspective on the role, team, or work, but they shall remain clearly tied to the same role-specific thread and shall not drift into generic unrelated networking language.
- **FR-EM-03M2A1 (Explicit Final-Touch Language For Follow-Up 3):** Follow-up `3` shall explicitly identify itself as the last follow-up in polite language, for example: `If now isn't the right time, I completely understand and this will be my last follow-up.` Earlier follow-ups shall not use final-touch wording that implies the sequence has already ended.
- **FR-EM-03M2A2 (Light Sequence-Specific Style Guidance):** The Codex follow-up drafter shall follow lightweight per-step style guidance rather than a rigid template:
  1. follow-up `1` should be the shortest message in the sequence, primarily a concise reminder with one clear CTA and no heavy mutual-fit paragraph
  2. follow-up `2` may include a short mutual-fit sentence and can be slightly stronger than follow-up `1`, but it shall remain concise and shall not restate the full original proof point
  3. follow-up `3` shall remain concise, respectful, and pressure-free while using explicit final-touch wording
- **FR-EM-03M2A2B (Single Compact Fit Reminder In Follow-Up 2):** Follow-up `2` may include one compact mutual-fit reminder drawn from the original outreach evidence. That reminder should stay brief and singular rather than turning into a renewed proof-heavy pitch. Follow-up `1` should remain mostly a reminder, and follow-up `3` should stay light rather than reopening fit in a heavy way.
- **FR-EM-03M2A2A (Follow-Up Paragraph Budget Rule):** Automatic follow-ups should default to two short paragraphs plus the fixed short signoff. A third short paragraph is allowed only when it is genuinely needed for clarity or smoothness; follow-ups shall not drift into long multi-paragraph restatements of the original email.
- **FR-EM-03M2A3 (Current Follow-Up CTA Rule):** The current follow-up CTA shall remain the existing simple ask for a brief `10-minute` conversation to hear the recipient's perspective on the role, team, or process. The follow-up worker does not need a separate alternate CTA policy in this build.
- **FR-EM-03M2A3A (Explicit Conversation Ask In Follow-Ups 1 And 2):** Follow-up `1` and follow-up `2` shall both keep the explicit `10-minute` conversation ask as the primary CTA. Follow-up `3` may phrase that same ask more softly, but the automatic sequence shall not switch follow-up `2` into a reply-only CTA by default.
- **FR-EM-03M2B (Codex-Only Follow-Up Production Rule):** Scheduled production follow-up drafting shall be Codex-only and shall fail closed if the configured Codex follow-up renderer cannot run, rather than silently falling back to the older deterministic/template-based follow-up body.
- **FR-EM-03M2B1 (Follow-Up Prompt Family Split Rule):** Scheduled Codex follow-up drafting shall use two bounded prompt families aligned with the original first-email posture:
  1. a technical follow-up prompt family for guidance-oriented threads
  2. a managerial follow-up prompt family for role-interest-oriented threads
  The worker shall not rely on one broad generic follow-up prompt that blurs those two production postures together.

- **FR-EM-03M3 (Follow-Up Template Grounding):** Follow-up drafting shall stay grounded in the original sent email thread, the linked role/company context when relevant, and the original resume/JD evidence when those artifacts exist. The drafter shall not collapse unrelated threads into one generic stock follow-up voice.
- **FR-EM-03M3J (Codex Follow-Up Rendering):** Follow-up body rendering shall use the approved Codex follow-up drafter. The worker may vary wording by sequence step and role context, but the draft must remain concise, grounded in allowed evidence, and pass validation before send.
- **FR-EM-03M3J1 (Bounded Follow-Up Input Pack Rule):** Before any follow-up `codex exec` call, deterministic runtime shall build a compact bounded follow-up evidence pack rather than passing full raw artifacts. The follow-up drafter shall not receive the full raw JD, full raw resume, full mailbox thread, or other large unrelated artifacts by default.
- **FR-EM-03M3J2 (Bounded Follow-Up Pack Contents):** At minimum the bounded follow-up pack shall include:
  1. the follow-up sequence step (`1`, `2`, or `3`)
  2. the full exact original sent email body as the primary narrative source
  3. the full exact text of any earlier sent follow-ups in that same thread when they exist
  4. a compact role/company summary
  5. the exact compact approved sender-evidence summary that was used for the original first email when that summary is recoverable
  6. the safe salutation and the minimal thread metadata needed for same-thread reply handling
- **FR-EM-03M3J2A (Compress Secondary Context, Not Prior Outbound Emails):** The bounded follow-up pack may compress secondary context such as role/company summary, sender-evidence summary, and thread metadata, but it shall not replace the original sent email body or earlier sent follow-up bodies with summaries by default. Prior outbound emails should remain full-text so the follow-up drafter can preserve exact tone, exact ask, and exact non-repetition constraints.
- **FR-EM-03M3J2B (Original Sender-Evidence Summary Preference):** Follow-up drafting shall prefer reusing the exact bounded sender-evidence summary that fed the original first email. If that exact bounded summary cannot be recovered, the worker may reconstruct a compact sender-evidence summary only from the original outreach context and the original linked resume evidence. It shall not rebuild the sender-evidence summary from the current latest resume/profile state by default.
- **FR-EM-03M3J3 (Bounded-Pack Failure-Closed Rule):** If the bounded follow-up pack cannot provide enough grounded context to draft a good follow-up safely, the worker shall hold, skip, or retry according to the follow-up retry rules rather than inflating the prompt with full raw artifacts.
- **FR-EM-03M3J3A (Structured Follow-Up Output Contract):** Follow-up `codex exec` calls shall use a JSON-only structured output contract rather than requesting one free-form full email blob. The structured output shall be small enough to validate deterministically before send.
- **FR-EM-03M3J3B (Follow-Up Output Boundary Rule):** Deterministic runtime shall own the same-thread subject behavior, the recovered salutation, and the fixed short signoff. The follow-up prompt shall draft only the body paragraphs plus compact debug fields needed for validation and audit. It shall not generate a new subject line or a new signoff block.
- **FR-EM-03M3J3C (Follow-Up 3 Deterministic Final-Touch Rule):** For follow-up `3`, deterministic runtime shall own the exact final-touch sentence `If now isn't the right time, I completely understand and this will be my last follow-up.` The renderer may validate or append that sentence as needed rather than leaving the final-stop wording fully freeform.
- **FR-EM-03M3J3D (Follow-Up Prompt Validation Guard):** Before a Codex-generated follow-up is accepted for send, deterministic validation shall confirm that the body does not introduce prohibited content such as a new subject, a new signoff block, reintroduced `Job Hunt Copilot` detail, resume/attachment references, quoted original-email content, or a heavy proof-point restatement that violates the follow-up rules.
- **FR-EM-03M3J3E (Follow-Up Output Shape Rule):** The follow-up structured output should expose the drafted body paragraphs plus compact machine-checkable fields needed for the follow-up review/debug note, such as whether role/company wording was explicit, whether a compact fit reminder was used, and whether a light technical career-path callback was used.
- **FR-EM-03M3J3F (Plain-Paragraph Follow-Up Prompt Rule):** The follow-up prompt should emit plain body paragraphs only. It shall not emit bullets, numbered lists, Markdown formatting, quoted thread blocks, or other presentation shapes that conflict with the compact plain-text follow-up style.
- **FR-EM-03M3J4 (Compact Follow-Up Why-Sent Note):** For each sent automatic follow-up, the worker shall persist a compact structured review/debug note summarizing why that follow-up was drafted and sent. This note should remain small and debuggable rather than becoming a full prompt dump or verbose reasoning trace.
- **FR-EM-03M3J4A (Follow-Up Why-Sent Fields):** The compact follow-up note should capture at least:
  1. the follow-up sequence step
  2. the preserved posture family (`technical` guidance-oriented or `managerial` role-interest-oriented)
  3. whether drafting used only the original outbound email or the original plus earlier sent follow-ups
  4. whether JD or resume-evidence fallbacks were used
  5. whether role/company wording was explicit or thread-implied
- **FR-EM-03M3K (Compact Reminder Shape):** Any role-fit reminder inside a role-interest follow-up shall stay to one compact sentence. It shall not expand into a background-fit list, proof paragraph, long metric claim, or generic skill dump.
...
- **FR-EM-03M3L1 (No Job-Hunt-Copilot Reintroduction By Default):** Follow-up emails shall not reintroduce or re-explain `Job Hunt Copilot`, its repo URL, or its implementation details by default when that material already appeared in the original first email. A brief mention is allowed only when it is genuinely necessary for thread clarity, and even then it should stay minimal rather than turning into a renewed project pitch.
- **FR-EM-03M3M (No Generic Follow-Up Fallback):** If the follow-up worker cannot produce a thread-specific, evidence-grounded follow-up that preserves the original thread posture, it shall hold or escalate the follow-up rather than sending a generic fallback body.
- **FR-EM-03M3M1 (Draft-Quality Retry Preference):** If the follow-up worker cannot draft a good Codex follow-up for the current sequence step, it should prefer holding or skipping that step for later re-evaluation rather than escalating immediately. Immediate escalation should be reserved for structural or persistent failures such as missing required original-body context, missing same-thread send metadata, or repeated drafting failure beyond the retry policy.
- **FR-EM-03M3M2 (Next-Cycle Draft Retry Rule):** When Codex follow-up drafting fails for an otherwise due follow-up step, the worker should retry that same step automatically on the next eligible worker cycle by default rather than waiting until the next business day or next cadence window, unless a more specific cooldown or structural stop condition applies.
- **FR-EM-03M3M3 (Bounded Automatic Draft Retry Rule):** Automatic Codex drafting retries for the same follow-up step shall be bounded at `3` attempts. After those `3` automatic retries are exhausted without producing an acceptable draft, the worker shall stop automatic retries for that step and leave it held, skipped, or reviewable according to the failure type rather than retrying indefinitely.
- **FR-EM-03M3M4 (Thread-Level Stop After Draft-Retry Exhaustion):** If any follow-up step exhausts its `3` automatic drafting retries without producing an acceptable draft, the automatic follow-up sequence for that entire thread shall stop. The worker shall surface the thread for owner review and shall not auto-materialize or auto-send later follow-up steps for that thread unless the owner explicitly resets or resumes it.
- **FR-EM-03M3D (Original Email Narrative Source Of Truth):** The original sent email body shall be the narrative source of truth for follow-up drafting. JD and resume evidence may support the reminder when needed, but the follow-up shall not change the story, claim a different fit, or introduce materially new positioning that conflicts with the original email.
- **FR-EM-03M3D1 (Prior Outbound Thread Context Rule):** When drafting follow-up `2` or follow-up `3`, the Codex follow-up drafter shall receive the original outbound email plus all earlier follow-ups already sent by the system in that same thread. This prior outbound thread context shall be used to avoid repeating the same wording, ask, or closing language across the sequence.
- **FR-EM-03M3D2 (No New Candidate Evidence Rule):** The follow-up drafter shall not introduce new candidate evidence, new project claims, or a materially new fit narrative that was not already available in the original outreach context. Follow-up drafting shall remain anchored to the original sent email, the associated role/JD context, and the resume evidence already linked to that original outreach.
...
- **FR-EM-03M3A (Follow-Up Missing-JD Fallback):** Missing JD or role-context artifacts shall not block an otherwise eligible follow-up. When JD context is unavailable, the worker may ground the follow-up in the original sent email body plus the original resume evidence and shall record that fallback in the follow-up plan review evidence.
- **FR-EM-03M3C (Original-Email-Only Grounding Fallback):** Missing tailored resume evidence shall not automatically block an otherwise eligible follow-up. If the original sent email body contains enough concrete context to support a short reminder, the worker may ground the follow-up in the original email body alone and shall record that fallback. If the original email body is too generic or unavailable, the worker shall hold or escalate rather than invent new fit language.
...
- **FR-EM-03M3E1 (Original Salutation Style Preference):** The follow-up drafter shall preserve the original salutation style whenever the recovered salutation is safe and normal enough for reuse, such as `Hi John,`, `Hello Kavita,`, or a generic `Hi,`. It may normalize the salutation only when the recovered original is clearly malformed, placeholder-like, email-address-like, or otherwise poor-quality enough to reduce email quality.
...
- **FR-EM-03M3H1 (Role/Company Mention By Sequence):** Follow-up `1` shall explicitly mention the role title and company name only for role-interest threads. Learning-first technical threads shall not be forced to restate a specific opening if the original thread was not anchored that way. Follow-up `2` and follow-up `3` may omit the explicit role/company restatement when the existing Gmail thread context already makes the reference clear and the shorter wording reads more naturally.
- **FR-EM-03M3H2 (Original Thread Posture Preservation):** A learning-first technical thread shall remain learning/guidance oriented in follow-up drafting, and a role-interest managerial thread shall remain role/company oriented. The follow-up worker shall not silently recast one posture into the other because later database context knows the job posting details.
- **FR-EM-03M3I (Ignore Signature For Follow-Up Grounding):** When extracting follow-up grounding signals from the original sent email, the worker shall ignore the sender signature and contact block so public-profile URL, phone, email, and signoff text do not contaminate follow-up phrasing.
...
- **FR-EM-03N0C (Superseded Unsent Draft Disposal Rule):** If a drafted follow-up becomes unsendable before send because a reply, bounce, stop condition, or other suppressing fact arrives first, that unsent draft should be treated as disposable. The worker may delete it or ignore it, but it shall not keep treating that draft as an active sendable artifact for the thread.
- **FR-EM-03N0A (Follow-Up Agent Review Evidence):** Before automatic send, the follow-up worker shall persist review evidence showing that the draft used the approved template, preserved the original thread posture, remained grounded in the original email plus any allowed JD/resume support, used no retired template wording, leaked no prohibited internal artifact text, saw no original-email bounce, saw no prior follow-up, and saw no inbound recipient reply after the original `sent_at` in the direct Gmail-thread reply check.
...
- **FR-EM-03N1A (No Automatic Reply-To-Reply Rule):** If the recipient replies in the thread, the follow-up worker shall not send any automatic email in response to that reply. Replied threads become owner-handled, and the owner will review and decide any further manual action.
- **FR-EM-03N1B (Contact-Wide Reply Suppression):** If a contact replies on any recorded outreach thread, the follow-up worker shall stop automatic follow-ups across all active threads for that same contact, not just the replied thread. After any inbound reply from that contact, further follow-up handling becomes owner-managed unless the owner explicitly resets the contact/thread state later.
- **FR-EM-03N1B1 (Any Reply Owner-Handoff Rule):** Any inbound reply shall hand the contact to the owner for manual handling, even when the worker cannot confidently classify that reply as positive, neutral, or negative. In ambiguous reply-classification cases, the worker shall still stop automatic outreach/follow-ups for that contact, but it shall not mark `do_not_contact` unless explicit negative-stop intent is actually clear.
- **FR-EM-03N1C (Negative Reply Do-Not-Contact Rule):** If a contact sends an explicit negative reply such as `stop emailing me`, `do not contact me`, `unsubscribe`, `not interested`, or equivalent contact-stop language, the system shall mark that contact as do-not-contact for all future outreach, not just for the current follow-up thread. That contact-level do-not-contact state shall block future automatic outreach and follow-ups unless the owner explicitly overrides it later.
- **FR-EM-03N1D (Positive Reply Owner-Handoff Rule):** If a contact gives a positive or open reply such as `sure`, `happy to chat`, `let's connect`, or equivalent interest language, the system shall stop all future automatic outreach and follow-ups for that contact and hand the relationship off to the owner for manual handling. Positive reply state is not do-not-contact, but it still removes that contact from automatic outbound sequences unless the owner explicitly re-enables automation later.
- **FR-EM-03N1E (Neutral Reply Owner-Handoff Rule):** If a contact gives a neutral or process-oriented reply such as `please send resume`, `check back later`, `we are reviewing`, or equivalent non-negative engagement, the system shall also stop all future automatic outreach and follow-ups for that contact and hand the relationship off to the owner for manual handling. Neutral replies are not do-not-contact, but they still remove that contact from automatic outbound sequences unless the owner explicitly re-enables automation later.
...
- **FR-EM-03N5B (No Resume Reintroduction In Automatic Follow-Ups):** Automatic follow-up emails shall not mention the tailored resume, attached resume, or resume-forwarding language by default. Resume discussion may occur later only in owner-handled manual reply flow, such as after a recipient explicitly asks for the resume.
- **FR-EM-03N5A (Single-Recipient Automatic Follow-Up Scope):** Automatic follow-ups shall apply only to original outreach threads whose original recipient envelope contains a single external target recipient. If the original thread was sent to multiple recipients, such as multiple `To` targets or any additional `Cc` recipients, the worker shall not auto-send a follow-up on that thread and shall instead leave it reviewable or skipped with a structured multi-recipient reason.
...
- **FR-EM-12C2 (Current Role-Split Drafting Strategy):** The current role-targeted drafting strategy supersedes the older shared deterministic template. The production path shall classify each recipient into either a managerial/application-style draft path or a technical/career-guidance draft path before body generation begins.
- **FR-EM-12C3 (Recipient Classification Source Rule):** Role-split classification shall prefer the Apollo-sourced current title for the selected contact, with recruiter/talent/sourcer/people-ops and leadership titles routing to the managerial path, and engineer/developer/architect/staff/principal/AI/ML titles routing to the technical path.
- **FR-EM-12C3A (Leadership-Adjacent Recipient Routing Rule):** Recipient classification shall also route leadership-adjacent technical titles such as `Founding Engineer`, `Technical Team Lead`, `Software Team Lead`, `Team Lead`, `Lead Software Engineer`, and comparable current-company technical-lead titles through the managerial path when they imply team or technical-lead responsibility. This routing still remains below true manager / founder / executive priority in contact ordering, but it shall not collapse those titles into the plain technical path by default.
- **FR-EM-12C3A (Recipient-Type Fallback Rule):** When the current title is sparse or ambiguous, recipient-type classification may fall back to the linked contact's canonical recipient type so the system can still choose a path without inventing person-specific background.
- **FR-EM-12C4 (Deterministic Input Bounding Rule):** Deterministic Python shall bound the drafting inputs before any `codex exec` call. At minimum this bounded evidence pack shall include the selected recipient path, target role/company context, and only the role-relevant JD or Apollo evidence needed for that path. For the managerial path, this bounded pack shall also include the compact sender core summary plus the bounded sender evidence pool. For the technical path, it shall include only the technical-path evidence needed to draft Paragraph 1 safely.
- **FR-EM-12C4A (Managerial JD Relevance Pack Rule):** For the managerial path, deterministic Python shall reduce the raw JD into a bounded relevance pack containing the exact role title, the most relevant engineering responsibilities, the most relevant technical requirements, and one short product/domain line when helpful, while excluding benefits, legal boilerplate, and broad marketing copy.
- **FR-EM-12C4B (Managerial Sender Evidence Pool Rule):** For the managerial path, deterministic Python shall pass a bounded sender evidence pool rather than the full raw resume. Each evidence item should carry one short proof line plus lightweight metric/theme tags so the drafter can select role-relevant evidence without turning the email into a resume dump.
- **FR-EM-12C4C (Technical Employment-History Pack Rule):** For the technical path, deterministic Python shall pass the full available Apollo employment-history summary for the recipient when that history exists. If only partial or current-role evidence is available, the input pack shall state that limited coverage explicitly rather than hallucinating missing career steps.
- **FR-EM-12C4D (DB-First Apollo Drafting Read Rule):** Role-targeted drafting shall read Apollo employment history and other Apollo person context from the structured database-backed provider stores first. Artifact reparsing may be used only as a fallback when the corresponding database-backed provider snapshot or employment-history rows are missing.
- **FR-EM-12C4E (Scheduled Codex Runtime Resolution Rule):** Scheduled role-split drafting shall resolve the `codex` executable through an explicit override or deterministic well-known local install paths and shall not rely solely on the ambient scheduler `PATH`.
- **FR-EM-12C4E1 (Scheduled Codex Launcher Dependency Rule):** Scheduled role-split drafting shall invoke `codex exec` with a deterministic runtime environment that can also resolve launcher dependencies such as `node` when the local `codex` entry point is a `#!/usr/bin/env node` script. The scheduler shall not assume that launchd or another bounded scheduler exposes a sufficient ambient `PATH` for those dependencies.
- **FR-EM-12C4E2 (Codex-Only Scheduled Role-Targeted Drafting Rule):** Scheduled role-targeted production drafting shall be Codex-only. If the role-split Codex renderer cannot be initialized or its scheduled invocation cannot run successfully, the scheduled role-targeted path shall fail closed and surface a drafting failure or incident rather than silently falling back to the older deterministic shared role-targeted renderer.
...
- **FR-EM-12C5D2 (Technical No-Autonomous-Workflow Sentence Rule):** The fixed technical Job Hunt Copilot paragraph shall not say that the email is a live example of an autonomous workflow and shall not use human-in-the-loop process language in the user-facing email body.
- **FR-EM-12C5D3 (Technical No-Availability-Paragraph Rule):** The fixed technical guidance ask shall not include explicit weekday availability or scheduling-range language in the current build.
- **FR-EM-12C5D4 (Technical Guidance Ask Rule):** The fixed technical ask should request the recipient's perspective on how they approached that kind of work or grew into it, rather than sounding like a generic scheduling request.
- **FR-EM-12C5D5 (Technical Guidance Ask Specificity Rule):** The fixed technical ask should explicitly frame the call as guidance for the sender's growth. It should ask what helped the recipient break into or grow in that kind of work, or what they would recommend someone at the sender's stage focus on, rather than sounding like a generic request for time.
- **FR-EM-12C5F (Technical No-Resume Rule):** The current technical path shall not mention an attached resume in the body and shall not attach the resume file at send time. Resume attachment handling remains available for the managerial path only.
- **FR-EM-12C5E (Technical Payload Normalization Rule):** Before persisting a technical-path draft, deterministic runtime may normalize non-semantic payload drift to preserve the existing contract. It may trim a longer-than-expected `paragraph_1_text` down to two sentences, append the fixed second-sentence intent when only one opener sentence is returned, and infer at least one `selected_career_steps` company from the bounded employment-history evidence when the model leaves that debug list empty. It shall not invent unsupported employers beyond the provided employment-history pack.
- **FR-EM-12C6 (Managerial Path Intent Rule):** The managerial path shall be a concise problem-solver note optimized for managers, recruiters, and routing-side contacts. It should frame the role in terms of likely team challenges, show role-relevant evidence from the sender, and ask for a brief 10-minute conversation to better understand the real problem space.
- **FR-EM-12C6A (Managerial Path Draft Boundary Rule):** In the current managerial path, `codex exec` shall draft the variable managerial content only: the role-alignment sentence inside Paragraph 1, the relevant-background bullets, and the debug fields. Deterministic Python shall render the greeting, the fixed bold proof-of-concept sentence, the fixed CTA paragraph, the bullet heading, the fixed subject, and the standard signature/attachment handling.
- **FR-EM-12C6B (Managerial Path Subject Rule):** The current managerial-path subject shall follow the fixed pattern `Interest in the <Role Title> role at <Company>`.
- **FR-EM-12C6B1 (Managerial Subject Normalization Rule):** Before rendering the managerial-path subject or subject-bound prompt text, deterministic runtime shall normalize obvious formatting artifacts out of the role title, such as a leading markdown/hash marker, so plain-text subjects do not leak source-formatting noise like `#Software Engineer`.
- **FR-EM-12C6C (Managerial Path Paragraph Shape Rule):** The current managerial path shall use this exact structure:
  1. `paragraph_1`: greeting plus a three-sentence opener, where sentence one is fixed as `I hope you're doing well.`, sentence two is a codex-drafted role-alignment sentence, and sentence three is the fixed bolded proof-of-concept offer sentence
  2. `relevant_background`: exactly three concise bullets rendered under `Relevant background from my side:`
  3. `cta_block`: a fixed CTA block containing the standalone resume-attachment line followed by the fixed three-sentence ask paragraph covering a 10-minute ask, the desire to understand the team's real challenges, and a forward-to-the-right-person note
- **FR-EM-12C6D2 (Managerial Dominant-Theme Opener Rule):** The managerial `role_alignment_sentence` shall choose one dominant role-fit theme that can stand as a coherent work identity for the opener. It shall not combine multiple unrelated JD signals merely to increase coverage.
- **FR-EM-12C6D4 (Managerial Legacy-Opener Removal Rule):** The live managerial drafting prompt and deterministic managerial rendering templates shall not instruct or require the legacy opener family that begins with phrases like `I came across the ... opening at ...`. Legacy opener suppression should be achieved at the drafting-source and rendering-source layers first; lint or validation blocks may remain only as defensive guardrails rather than as the primary mechanism that keeps production managerial drafts on the current wording family.
- **FR-EM-12C6E (Managerial Relevant-Background Bullet Rule):** Managerial `relevant_background` bullets shall be chosen dynamically from the sender's real resume evidence, including both experience and project sections when relevant. They should prefer one strongest role-relevant professional proof, one second strong professional or systems proof, and an optional `Job Hunt Copilot` bullet only when that project materially strengthens fit for the role.
- **FR-EM-12C6E1 (Managerial Profile-Evidence Retrieval Rule):** Before `codex exec` drafts managerial `relevant_background` bullets, the system shall run a bounded profile-evidence retrieval step. That retrieval step shall use the target role title, the bounded JD relevance pack, and the selected managerial role theme to fetch the most relevant sender evidence instead of passing a generic untargeted evidence pool directly into the prompt.
- **FR-EM-12C6E1A (Managerial-Only RAG Scope Rule):** In the current build, the bounded profile-evidence retrieval layer applies only to the managerial first-email path. The technical-path first email shall keep its current bounded drafting path and shall not switch to this profile-evidence RAG flow yet.
- **FR-EM-12C6E1B (Managerial Relevant-Background-Only RAG Rule):** In the first managerial profile-evidence RAG build, retrieval shall improve only the `relevant_background` section and its provenance/debug path. The current managerial `role_alignment_sentence` should continue to use the existing bounded JD/theme flow rather than expanding this change into a full managerial prompt redesign.
- **FR-EM-12C6E2 (Managerial Retrieval Corpus Scope Rule):** The managerial profile-evidence retrieval step shall search a broader reusable profile-evidence corpus rather than only the current tailored-resume bullets. In the current build, that corpus should be able to include resume experience proof points, resume project proof points, `Job Hunt Copilot` proof points, and other approved outreach-safe profile snippets when present.
- **FR-EM-12C6E2A (Managerial Retrieval Corpus Source Rule):** The reusable profile-evidence corpus shall be built only from approved master profile sources. Historical tailoring artifacts, prior outreach drafts, and prior sent outreach bodies shall not become canonical retrieval inputs for first-email managerial drafting.
- **FR-EM-12C6E2B (Managerial Curated Evidence-Source Rule):** In the current build, the reusable profile-evidence corpus shall be generated from one curated canonical evidence source that the owner can inspect and edit directly. Raw resume/profile documents may remain upstream references, but they should not be the direct runtime chunk format for managerial retrieval.
- **FR-EM-12C6E2B1 (Managerial Curated Evidence-Source Format Rule):** In the current build, the curated canonical evidence source should be one version-controlled YAML artifact so the owner can edit evidence chunks directly while the builder still receives deterministic structured input.
- **FR-EM-12C6E2B1A (Managerial Curated Evidence-Source Shape Rule):** The curated YAML source should use one simple top-level chunk collection rather than many source-specific sections. The first build should favor one straightforward builder input shape over a heavily nested authoring format.
- **FR-EM-12C6E2B2 (Managerial Curated Evidence-Source Location Rule):** The curated canonical evidence-source YAML shall live in a version-controlled project asset/config path rather than under `ops/`. `ops/` remains for generated runtime mirrors and diagnostics, not for the owner-edited source of truth.
- **FR-EM-12C6E2C (Managerial Evidence-Chunk Schema Rule):** In the current build, each canonical profile-evidence chunk should use a minimal stable schema containing:
  1. `evidence_id`
  2. `text`
  3. `source_type`
  4. `evidence_type`
  ...
  6. `theme_tags`
  7. `strength`
  This schema is the retrieval contract for the first managerial profile-evidence RAG build.
- **FR-EM-12C6E2D (Managerial Atomic Evidence-Chunk Rule):** Each canonical profile-evidence chunk shall represent one atomic proof point. The retrieval corpus shall not use long paragraphs, stitched multi-claim summaries, or bare standalone skill lists as chunks. When one source line contains two materially different proof points, the curated canonical source should split them into separate chunks.
- **FR-EM-12C6E2E (Managerial Evidence `source_type` Rule):** In the current build, canonical managerial profile-evidence chunks shall use only these `source_type` values:
  1. `resume_experience`
  2. `resume_project`
  3. `job_hunt_copilot`
  4. `tennis_augmented`
  5. `education`
  `resume_project` refers to proof points from the ordinary projects section of the approved master profile. `job_hunt_copilot` and `tennis_augmented` remain explicit first-class source types rather than being folded into generic project chunks so their retrieval behavior can be controlled and audited separately.
- **FR-EM-12C6E2F (Managerial Evidence `evidence_type` Rule):** In the current build, canonical managerial profile-evidence chunks shall use only these `evidence_type` values:
  1. `achievement`
  2. `project`
  3. `system`
  4. `reliability`
  5. `stakeholder`
  6. `skill_anchor`
  `skill_anchor` should remain a short capability anchor grounded in real approved profile evidence and should not replace stronger proof-backed chunks when direct role-relevant proof exists.
- **FR-EM-12C6E2F1 (Managerial `skill_anchor` Discipline Rule):** `skill_anchor` chunks should be used sparingly. They may help preserve important technical signal when stronger retrieved proof would otherwise omit a key capability, but they should not outrank stronger outcome-backed chunks by default.
- **FR-EM-12C6E2G (Managerial Tag Normalization Rule):** `skill_tags` and `theme_tags` shall be stored as normalized lowercase stable tokens so structured retrieval remains inspectable and deterministic. The first build should avoid free-form sentence-like tags in those fields.
- **FR-EM-12C6E2H (Managerial Evidence Strength Rule):** `strength` shall use a simple bounded integer scale from `1` to `5`, where higher values represent stronger, clearer, more reusable proof for outreach retrieval. The first build should treat `5` as strongest production-grade or outcome-backed evidence and `1` as weak supporting evidence.
- **FR-EM-12C6E2I (Managerial Evidence Storage Rule):** The reusable managerial profile-evidence corpus shall use SQLite as canonical runtime storage in a dedicated `profile_evidence_chunks` table. The owner-edited upstream source should live as one curated version-controlled evidence-source artifact, and the built runtime corpus should also be mirrored to a human-readable inspection artifact under `ops/` for debugging.
- **FR-EM-12C6E2I1 (Managerial Corpus Mirror Location Rule):** The generated human-readable mirror of the built managerial evidence corpus should live under a dedicated `ops/profile-evidence/` runtime path so inspection artifacts stay separate from the version-controlled curated source of truth.
- **FR-EM-12C6E2J (Managerial Evidence Refresh Rule):** The canonical profile-evidence corpus shall refresh only through an explicit corpus-build step. The current build shall not rebuild or re-chunk the corpus opportunistically during every outreach draft.
- **FR-EM-12C6E2J0 (Managerial Corpus-Build Validation Rule):** The explicit corpus-build step shall validate the curated YAML source before writing canonical runtime state. At minimum it should reject duplicate `evidence_id` values, missing required fields, invalid `source_type` or `evidence_type` values, malformed tag lists, and `strength` values outside the supported bounded scale.
- **FR-EM-12C6E2J1 (Managerial Evidence Refresh Stability Rule):** Normal supervisor, drafting, and send execution paths shall read the current built corpus as-is. They shall not attempt to mutate, regenerate, or auto-heal the managerial evidence corpus in the middle of live pipeline execution.
- **FR-EM-12C6E2J2 (Managerial Missing-Corpus Fail-Closed Rule):** If the managerial profile-evidence corpus is missing, unreadable, or otherwise unavailable at draft time, the current build shall fail closed for managerial first-email drafting rather than silently falling back to the older generic sender-evidence pool.
- **FR-EM-12C6E3 (Managerial Structured-Retrieval v1 Rule):** The first build of the managerial profile-evidence retriever shall use bounded structured retrieval and ranking rather than a vector database or LLM reranker. Retrieval quality should come from controlled evidence chunk structure plus deterministic ranking over role/theme relevance signals.
- **FR-EM-12C6E3A (Managerial Retrieval Scoring Rule):** The first structured managerial retriever shall rank chunks using the current managerial role theme, the bounded JD relevance pack, the role title, normalized tag overlap, light lexical overlap with selected JD signals, and chunk `strength`. The first build shall not use embeddings or an LLM reranker.
- **FR-EM-12C6E3A1 (Managerial Deterministic Retrieval Ordering Rule):** When two evidence chunks receive the same retrieval score, the current build shall use stable deterministic tie-breakers so the same corpus and same JD context do not produce arbitrary evidence-order drift across repeated drafts.
- **FR-EM-12C6E3B (Managerial Retrieval Diversity Rule):** The retrieved prompt evidence pack should preserve diversity rather than returning near-duplicate chunks. When the corpus supports it, the bounded pack should contain at least:
  1. one direct role-theme proof chunk
  2. one strong technical or systems proof chunk
  3. one complementary supporting chunk
- **FR-EM-12C6E3B1 (Managerial Technical-Signal Density Rule):** For technical, AI/ML, data, platform, backend, or systems-heavy managerial roles, the retrieved evidence pack and the final three `relevant_background` bullets should preserve visible technical signal. Stakeholder-oriented support may appear, but it should not dominate the section when stronger technical proof exists.
- **FR-EM-12C6E3C (Managerial Generic-Reliability Demotion Rule):** Reliability, monitoring, uptime, and generic scale chunks should be demoted when the current role theme is not reliability- or operations-heavy and more directly theme-aligned chunks exist. They may still appear as secondary support, but they shall not dominate the retrieved evidence pack for AI/workflow/client-delivery roles merely because they contain large metrics.
- **FR-EM-12C6E3D (Managerial Project-Chunk Discipline Rule):** Project-like chunks, including `resume_project`, `job_hunt_copilot`, and `tennis_augmented`, may compete in retrieval, but they should not crowd out stronger directly relevant professional evidence by default. In the first build, the final three `relevant_background` bullets should usually contain no more than one project-derived chunk unless the role is itself strongly project-aligned and the retrieved evidence shows that project proof is among the strongest role-fit signals.
- **FR-EM-12C6E4 (Managerial Retrieval Boundary Rule):** For one managerial draft, the retrieval layer shall fetch a wider bounded candidate set of up to `8` profile-evidence chunks, pass no more than the top `5` retrieved chunks into the managerial Codex prompt, and require Codex to choose exactly `3` final `relevant_background` bullets from that bounded retrieved set.
- **FR-EM-12C6E4A0 (Managerial Retrieval Prompt-Boundary Rule):** The managerial Codex prompt shall stay bounded in the same spirit as the current working role-split pipeline. It may see the bounded JD relevance pack, the selected managerial role theme, the compact sender-core summary, and no more than the top `5` retrieved evidence chunks. It shall not receive the full profile-evidence corpus, full raw resume/profile documents, or broad untargeted historical context.
- **FR-EM-12C6E4A00 (Managerial Retrieved-Chunk Prompt Shape Rule):** For each retrieved evidence chunk handed into the managerial prompt, the current build should expose only the bounded retrieval fields: `evidence_id`, `text`, `source_type`, `evidence_type`, `skill_tags`, `theme_tags`, and `strength`. Raw source documents, builder-only metadata, and unrelated corpus internals shall stay out of the prompt.
- **FR-EM-12C6E4A (Managerial Retrieval Prompt Provenance Rule):** Each retrieved chunk handed into the managerial Codex prompt shall retain its `evidence_id` so the generated payload can name which evidence items it used. The managerial structured output should return bullet-aligned provenance alongside `relevant_background` rather than one loose flat evidence-id list.
- **FR-EM-12C6E4A2 (Managerial Provenance Cardinality Rule):** The managerial structured output should return exactly three provenance entries aligned with the three final `relevant_background` bullets so downstream validation can map each rendered bullet to its supporting retrieved evidence.
- **FR-EM-12C6E4A1 (Managerial Bullet-to-Evidence Mapping Rule):** Each final managerial `relevant_background` bullet should map to one primary retrieved `evidence_id`. A bullet may additionally cite one secondary retrieved `evidence_id` only when both chunks are tightly related parts of the same proof story. The current build shall not allow one bullet to blend several unrelated chunks just to sound broader.
- **FR-EM-12C6E4B (Managerial Relevant-Background Grounding Rule):** Each final managerial `relevant_background` bullet shall trace to at least one retrieved `evidence_id`. The drafter may compress or lightly rephrase retrieved evidence, but it shall not invent unsupported claims or blend unrelated chunks into one bullet in a way that loses provenance.
- **FR-EM-12C6E4C (Managerial Weak-Evidence Fail-Closed Rule):** If the retrieval stage cannot produce enough grounded evidence to support three credible managerial `relevant_background` bullets, the drafting flow shall fail closed rather than padding the section with weak generic bullets. The current build should surface that case as a draftable review/failure condition instead of silently emitting low-relevance background.
- **FR-EM-12C6E4C1 (Managerial Two-Strong-One-Weak Rejection Rule):** When retrieval yields only one or two strong role-relevant chunks and the remaining available chunks are only weak generic support, the managerial draft shall be rejected as under-grounded rather than allowing Codex to manufacture a third weak filler bullet.
- **FR-EM-12C6F (Managerial Job Hunt Copilot Rule):** In the managerial path, `Job Hunt Copilot` shall usually appear only as one concise builder-signal bullet and may include the repo URL in that same bullet sentence. The managerial path should not say that the workflow runs autonomously or that the email itself is a live example of the workflow.
- **FR-EM-12C6F1 (Managerial Weak Job Hunt Copilot Suppression Rule):** In the managerial path, `Job Hunt Copilot` should not remain in the final `relevant_background` bullets when the dominant role theme is not clearly AI, agentic systems, workflow automation, or closely related production-AI enablement and stronger retrieved professional evidence is available.
- **FR-EM-12C6G (Managerial PoC Emphasis Rule):** The managerial opener shall include this exact proof-of-concept offer sentence in bold formatting: `If helpful, I'd be happy to build a small proof of concept based on my understanding of the challenges the team is working on and share the repo.`
- **FR-EM-12C6G0 (Managerial Plain-Text / HTML Emphasis Rule):** The fixed managerial proof-of-concept sentence shall remain plain text in the stored plain-text body and shall receive bold emphasis only in rendered HTML. Raw markdown emphasis markers such as `**...**` shall not leak into the plain-text alternative.
- **FR-EM-12C6G1 (Managerial Fixed CTA Rule):** The managerial CTA paragraph shall stay fixed in wording for the current build as:
...
- **FR-EM-12C6G2A (Managerial Posting-Link Placement Rule):** When a managerial-path draft includes a public posting URL, deterministic Python shall render it as one standalone line in the body using the exact prefix `Posting link:` immediately after the fixed bold proof-of-concept sentence and before the `Relevant background from my side:` heading.
- **FR-EM-12C6G2B (Managerial Bullet Rendering Rule):** The managerial `relevant_background` section shall render as an actual bullet list in both plain-text and HTML email bodies. Downstream HTML rendering shall not collapse the heading-plus-bullets into one paragraph.
...
- **FR-EM-12C6H (Current Role-Split Word Targets):** For the current build, the technical-path body should target roughly 165 to 190 words and the managerial-path body should target roughly 185 to 215 words, excluding the signature block.
- **FR-EM-12C6I (Structured Debug Payload Rule):** Each role-split draft shall persist a reviewable draft-debug artifact alongside the draft artifacts. For the technical path this should at minimum include the selected career-step companies. For the managerial path this should at minimum include the selected JD signals and selected sender-evidence signals actually used in the draft.
- **FR-EM-12C6I1 (Managerial RAG Debug Payload Rule):** In the managerial profile-evidence RAG build, the structured managerial output should keep `selected_jd_signals` and replace the old loose `selected_resume_signals` debug concept with bullet-aligned retrieved-evidence provenance that identifies which `evidence_id` backed each final `relevant_background` bullet.
...
- **FR-EM-12C6J4 (Role-Targeted Codex Fail-Closed Rule):** Scheduled or autonomous role-targeted original-email drafting shall fail closed when the active Codex role-split renderer cannot produce a valid draft. The runtime shall not fall back to the older deterministic role-targeted template, the older `Job Hunt Copilot`/autonomous-workflow pitch block, or any subject-line-compatible legacy body assembler for production sends.
- **FR-EM-12C6J4A (Role-Targeted Explicit-Origin Rule):** Role-targeted draft-origin metadata shall come from explicit renderer/debug provenance rather than heuristic subject-line inference. A draft that did not actually come from a successful Codex role-split render shall not be labeled or persisted as `codex_role_split`.
- **FR-EM-12C6J4B (Built-In Deterministic Outreach Drafter Retirement Rule):** The built-in deterministic outreach drafter shall be fully retired from the production/runtime codepath for both `role_targeted` and `general_learning` outreach generation. The runtime shall not keep a built-in deterministic original-email renderer available as a fallback resolver, environment-toggle resolver, or helper path that can still be selected by ordinary operator/startup configuration.
- **FR-EM-12C6J4C (Apollo Recipient-Type Refresh Rule):** When Apollo enrichment refreshes a posting-linked contact with a clearer current-company title, the runtime shall also refresh the linked `job_posting_contacts.recipient_type` and relevance reason from that latest Apollo title before send-slice selection. Stale source-time classifications like `other_internal` shall not keep a clearly role-adjacent engineer or manager out of the automatic send frontier once Apollo has already resolved that contact more precisely.
- **FR-EM-12C6K (Role-Targeted Original Lint Gate Rule):** Before persisting or refreshing a role-targeted original email draft, deterministic runtime shall run a lint gate over the rendered subject/body pair. The lint gate shall fail closed on raw plain-text markdown emphasis leakage, disallowed control characters, obvious subject formatting artifacts, blocked technical-path legacy phrases, and major word-budget overshoot.
- **FR-EM-12C6K1 (Role-Targeted Original Lint Scope Rule):** In the current build, this lint gate applies to the active role-split original-outreach drafting path only and shall not expand older manual helper paths by accident.
- **FR-EM-12C6K2 (Role-Targeted Word-Budget Counting Rule):** The role-targeted original word-budget lint shall approximate human-readable email length rather than penalizing transport details. For the current build, the count shall ignore signature-block inflation from raw URLs, email addresses, phone-number formatting, and the standalone `Posting link:` URL line when evaluating major word-budget overshoot. Slight tokenization variance from those transport lines shall not block an otherwise on-target draft.
- **FR-EM-12C6K3 (Legacy Autonomous-Workflow Phrase Block Rule):** The role-targeted original lint gate shall reject any rendered body that reintroduces the old deterministic outreach copy such as `Lately, I have been spending time sharpening my Agentic AI skills`, `This email is a live example of that workflow`, or equivalent autonomous-workflow pitch language that the current role-split system no longer allows.
...
- **FR-EM-18D1 (Shared Focus Consistency Rule):** In the current shared role-targeted template, the forwardable snippet focus shall stay consistent with the same accepted JD-grounded work-area choice used for the opener, allowing only a compact wording adjustment for snippet readability.
...
- **FR-OPS-17J1B (Bounded Pre-Cutover Posting Archive Rule):** The current build shall support a bounded operator archive cutover for postings selected by a `created_at` cutoff. That cutover shall preserve history rather than hard-delete rows, stamp `job_postings.archived_at`, move any still-active selected posting to `abandoned`, retire any non-terminal posting-scoped `pipeline_runs`, retire unsent posting-linked drafts so they cannot later resume, mark pending follow-up plans as skipped, and mark pending expert-review packets as reviewed so the archived backlog no longer appears actionable in the normal unattended queues.
...
- **FR-OPS-29O7A (Shared Send-Window Dashboard Visibility):** Startup/dashboard and relevant worker diagnostics shall surface the currently active 2-hour shared send window in `America/Phoenix`, including whether the lane is follow-up-preferred or original-outreach-preferred and whether fallback is currently in effect. The owner should be able to inspect why a queue is waiting without reconstructing the window schedule manually.
- **FR-OPS-29O7B (Next Window Preview):** The shared send-window diagnostics should also surface the next upcoming 2-hour local window and its preferred queue, so the owner can see when the lane preference will flip next without manually computing the schedule.
...
1. Given a promoted role-targeted posting, the system first preserves all source-seeded contacts provided by Jobright before running any Apollo manager-expansion search.
...
4. `people_search_result.json` is produced and preserves the resolved company record, applied search filters, the source-seeded contacts, and any Apollo people-search candidate list returned by people search, including manager-expansion candidates when present.
...
24. If a shortlisted candidate becomes a terminal dead end at the enrichment boundary and will not continue into email discovery or outreach, that candidate is dropped from canonical shortlist state rather than being retained as dead contact state.
25. Given linked contact input, the system returns a discovered working email or an explicit unresolved/not-found outcome.
...
36. The autonomous Jobright mode can use Apollo to add shortlisted current-company manager / executive / founder contacts under the manager-expansion policy when useful for outreach.
...
17. The default sequence is original outreach plus up to 3 follow-ups, with business-day spacing of 3, 4, and 5 business days between touches, using a Codex-based follow-up drafter that preserves the original thread posture and uses the short follow-up signature.
...
20A. Scheduled role-targeted production drafting is Codex-only and fails closed if the role-split Codex renderer cannot run, rather than silently falling back to the older deterministic shared renderer.
21. In the current role-split build, technical contacts receive a four-paragraph career-guidance email with a fixed subject, codex-generated Paragraph 1, fixed sender/project paragraphs, a guidance-oriented 10-minute ask, and no resume attachment.
...
28. The autonomous mode uses source-seeded contacts first and Apollo only to enrich emails or add separately shortlisted current-company leadership contacts.
...
8. Source-seeded contacts are preserved first, Apollo is used to enrich their emails, and Apollo manager-expansion may add adaptive-cap manager / executive / founder contacts without any separate minimum-total-contact top-up rule.
