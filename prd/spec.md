...
- **FR-EM-03M0A1 (No Extra Subjective Delay Buffer):** The configured `4 / 5 / 7` business-day cadence is the full timing buffer for automatic follow-ups. Once a thread crosses the applicable cadence threshold and is inside the allowed send window, it is eligible; the worker shall not apply an extra subjective `too recent` delay layer beyond the configured cadence.
- **FR-EM-03M0A2 (No Weekend-Adjacency Special Case):** The follow-up worker shall not apply special timing treatment just because the original email was sent near a weekend. Business-day counting alone governs the due date, so a Friday original may yield a Thursday follow-up `1` exactly by the normal cadence math.
- **FR-EM-03M0A3 (Original Send-Time Non-Blocking Rule):** An original first email sent outside the preferred business-hours window shall not by itself block a later automatic follow-up. If the follow-up is otherwise due and the follow-up send itself occurs inside the allowed follow-up send window, the thread remains eligible.
...
- **FR-EM-03M0A (Business-Hours Send Window):** After a follow-up becomes eligible, the follow-up worker shall send it only within the owner-approved business-hours window of `5:00 AM` through `5:00 PM` in `America/Phoenix`. Follow-up sends shall not go out overnight or outside that business-hours window.
...
- **FR-EM-03M0B1A (New Outreach Priority In Shared Lane):** When both a new role-targeted outreach email and a due follow-up email are simultaneously eligible to send in the shared outbound lane, the system shall prioritize the new role-targeted outreach email first. Due follow-ups shall wait for the next available turn that satisfies the shared pacing gate.
...
- **FR-EM-03M0E (Multiple Due Follow-Ups Allowed):** The follow-up worker may send multiple due follow-ups in the same local day. It shall continue working through due follow-ups in oldest-first order as long as each send still satisfies the shared outbound pacing gate, the `5:00 AM - 5:00 PM` window, and all other follow-up safety rules.
