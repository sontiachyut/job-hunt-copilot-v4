...
- **FR-EM-12C5C (Technical Path Paragraph-1 Rule):** Technical Paragraph 1 shall stay at exactly two sentences, begin with `I came across your LinkedIn profile and admired your path` or a clean equivalent that preserves the same admiration tone, prefer two to three past company transitions plus the recipient's exact current title and company when available, and avoid restating the fixed later paragraphs.
- **FR-EM-12C5C1 (Technical Path Clean Career-Step Fallback Rule):** When the recipient's available career history is partial, repetitive, or mostly one-company growth, the technical opener shall fall back to a clean current-role growth sentence rather than forcing a transition chain. It shall not emit placeholder ellipses, malformed `path... from` phrasing, or awkward same-company constructions like `from your long tenure at X into your current role at X`.
- **FR-EM-12C5D (Technical Path Fixed Paragraphs Rule):** Technical Paragraph 2 through Paragraph 4 shall remain deterministic and fixed in wording for the current build, but they shall now read as a shorter, cleaner technical-guidance note rather than a long self-explanation. Paragraph 2 shall be one concise background sentence anchored on the MSCS plus one strong production proof. Paragraph 3 shall be one concise Job Hunt Copilot builder-signal sentence with the repo folded into the project mention rather than a standalone `repo is here` sentence. Paragraph 4 shall be one concise guidance ask centered on the recipient's perspective.
...
- **FR-EM-12C5D2 (Technical Job Hunt Copilot Restraint Rule):** The fixed technical Job Hunt Copilot paragraph shall stay as supporting builder context rather than the emotional center of the email. It shall not describe the email itself as a live example of an autonomous workflow, shall not explain HITL review mechanics, and shall not add more than one short follow-on sentence beyond the project introduction.
- **FR-EM-12C5D3 (Technical Guidance Ask Restraint Rule):** The fixed technical guidance ask shall stay low-friction and concise. It shall ask for a brief `10-minute` conversation without listing weekday availability windows, calendar-like constraints, or other scheduling-detail sentences in the body.
- **FR-EM-12C5D4 (Technical Guidance Ask Recipient-Perspective Rule):** The fixed technical guidance ask shall ask for the recipient's perspective on how they approached this kind of work or grew into it, rather than centering the sender's job-hunt logistics.
- **FR-EM-12C5D5 (Technical Guidance Ask Specificity Rule):** The fixed technical guidance ask should explicitly request guidance for the sender's growth, such as what helped the recipient grow into this kind of work or what they would recommend focusing on at the sender's current stage, while still staying concise and low-pressure.
...
- **FR-EM-12C5F (Technical No-Resume Rule):** Technical-path original outreach shall not mention an attached resume in the body and shall not attach the resume at send time. Resume attachment remains allowed for the managerial path only in the current build.
...
- **FR-EM-12C6B1 (Managerial Subject Normalization Rule):** Before rendering the managerial-path subject, deterministic runtime shall normalize obvious formatting artifacts from the role title, including leading Markdown heading markers like `#`, stray bullet markers, and extra surrounding punctuation or whitespace. The subject shall preserve the canonical role meaning but must not send raw formatting tokens.
...
- **FR-EM-12C6E1 (Managerial Background Relevance Density Rule):** Managerial `relevant_background` bullets shall maximize relevant technical signal density rather than generic impressiveness. When stronger role-aligned professional proof exists, generic scale, uptime, or monitoring bullets shall not crowd it out merely because they carry larger headline metrics.
...
- **FR-EM-12C6F1 (Managerial Job Hunt Copilot Admissibility Rule):** A managerial `Job Hunt Copilot` bullet is allowed only when it directly reinforces the dominant role-fit theme or a clearly relevant adjacent technical theme such as applied AI workflows, workflow automation, agent orchestration, or production AI enablement. It shall not displace stronger professional evidence for unrelated roles merely because it is recent or novel.
...
- **FR-EM-12C6G0 (Managerial Plain-Text Emphasis Rendering Rule):** The managerial proof-of-concept sentence is a semantic emphasis requirement, not a literal Markdown requirement. Plain-text bodies shall render the sentence as normal text without raw `**` markers, while HTML rendering may apply `<strong>` emphasis.
...
- **FR-EM-12C6K (Role-Split Original Lint Gate Rule):** Before a role-targeted original draft is persisted or refreshed into `generated`, deterministic runtime shall run a rendered-draft lint gate over the final subject, plain-text body, and HTML body. The lint gate shall fail closed for structural or quality defects that are deterministic to detect, including raw Markdown emphasis leakage in plain text, disallowed control characters, subject formatting artifacts, banned technical-path autonomy/scheduling phrases, and major word-budget overshoot.
- **FR-EM-12C6K1 (Role-Split Original Lint Scope Rule):** The original-draft lint gate applies only to role-targeted original outreach in this build. Follow-ups may gain a similar lint gate later, but are not part of this change.
...
15. In autonomous role-targeted mode, the initial enrichment shortlist is capped at 10 contacts and is limited to manager-adjacent and technical contacts, with managers preferred first, then more senior engineers, then other software engineers.
