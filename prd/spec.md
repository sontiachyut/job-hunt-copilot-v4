...
- **FR-SYS-38B3G (Orphaned Send-Stage Run Recovery Rule):** If a role-targeted posting still has send-stage work actionable now but no non-terminal posting-scoped `pipeline_run` remains for that posting, the supervisor shall recreate one durable role-targeted run at the correct downstream boundary instead of leaving the posting stranded behind ordinary discovery backlog. In the current role-targeted flow, actionable `ready_for_outreach` or `outreach_in_progress` postings shall recover at `sending`, and actionable feedback-only postings shall recover at `delivery_feedback`.
...
- **FR-EM-03L4B (Next-Step-Only Plan Materialization):** The follow-up worker shall materialize only the next allowed follow-up step when it becomes relevant. It shall not pre-create all three possible follow-up sequence rows upfront for a thread. Follow-up `2` becomes materializable only after follow-up `1` has been sent and the `5` business-day timer becomes relevant; follow-up `3` becomes materializable only after follow-up `2` has been sent and the `7` business-day timer becomes relevant.
...
- **FR-EM-03M (Current Follow-Up Cadence):** For unreplied sent outreach, the system shall support the original email plus up to three automatic follow-ups per original outreach thread. The current default cadence is:
  1. follow-up `1` becomes eligible after `4` business days from the original sent email
  2. follow-up `2` becomes eligible after `5` business days from follow-up `1`
  3. follow-up `3` becomes eligible after `7` business days from follow-up `2`
...
- **FR-EM-03M0TZ2 (Cadence Days Versus Send Windows Separation Rule):** The follow-up `4 / 5 / 7` cadence continues to use business-day math even though the shared outbound send-window policy runs all 7 days. Business-day cadence determines when a follow-up becomes due; the 24-hour shared send-window policy determines when a due follow-up may actually take a send turn.
...
- **FR-EM-03M0A0 (No Historical Step Jumping Rule):** When an older eligible original outreach thread has aged far past the normal cadence, the worker shall still begin with follow-up `1`. It shall not jump directly to follow-up `2` or follow-up `3` solely because more calendar time has elapsed since the original email. Later steps become eligible only after the prior follow-up in that same thread has actually been sent and its own `5` or `7` business-day timer has then elapsed.
- **FR-EM-03M0A0A (No Extra Subjective Delay Buffer):** The configured `4 / 5 / 7` business-day cadence is the full timing buffer for automatic follow-ups. Once a thread crosses the applicable cadence threshold and is inside the currently allowed shared send window, it is eligible; the worker shall not apply an extra subjective `too recent` delay layer beyond the configured cadence.
...
17. The default sequence is original outreach plus up to 3 follow-ups, with business-day spacing of 4, 5, and 7 business days between touches, using a Codex-based follow-up drafter with the short follow-up signature.
