...
- **FR-EM-12C4E (Scheduled Codex Binary Resolution Rule):** Scheduled role-split drafting shall resolve the `codex` executable through an explicit override or deterministic well-known local install paths and shall not rely solely on the ambient scheduler `PATH`.
