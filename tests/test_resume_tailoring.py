...
def test_bootstrap_tailoring_run_creates_eligibility_artifact_and_run(tmp_path):
    ...
    assert artifact_payload["job_posting_id"] == "jp_test"
    assert artifact_payload["eligibility_status"] == ELIGIBILITY_STATUS_ELIGIBLE
    assert artifact_payload["bootstrap_ready"] is True
    assert artifact_payload["active_resume_tailoring_run_id"] == result.run.resume_tailoring_run_id
